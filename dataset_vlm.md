...
- [Shared execution contract](docs/dataset_multihost_runbook.md): Isaac Sim Replicator RGB, aligned optical-Z depth, validity, instance IDs, calibration and robot evidence; all 144 complete labeled background plants; one eligible petiole and a 9mm cut point; a common 679-file numerical source lock, global duplicate checks and visual review.
- The initial background layout is shared. Generator seeds alone do not establish complete scene or image uniqueness. Fresh background geometry requires a new full census/catalogue and qualification.
- The last server reports had no production collection: thor1 was installing Isaac Sim, thor3 preparing a two-image Linux preview. Linux owner/GPU/provenance adapters and remote coordinator connections still require qualification. Root SSH connections failed; remote production has not been verified.
- One local coordinator runs on loopback `127.0.0.1:8769`, initialized from 559 existing unique images (491 accepted + 68 raw-only). The committed baseline is `configs/dataset_capture/baseline_inventory.jsonl`. Runtime service receipt: `data/sim_data/diagnostics/native848_multihost_coordinator_20260918_v2/service.json`. Never create independent copied coordinator databases.
- The first coordinated original-plant batch saved six frames; none passed all width, darkness and background criteria. The following donor19 batch saved 16 frames across four morphologies: 15 had unique cut-point candidates, 15 triggered perceptual-similarity review holds, and zero were exact RGB duplicates. One unflagged, visually reviewed frame with 48.45% background coverage was added to the release. All 15 similarity holds remain excluded.
- Latest release: `C:/Users/USER/tomato-vlm-data-20260917/training_exports/tomato_cutpoint_848x408_delivery492_20260918_v1`: 442 original + 50 generated images; 396 TRAIN / 82 validation / 14 test; 13 original donors, 20 source targets and 15 donor-derived morphologies. All previous 491 entries were preserved and every model input was verified. Generated morphologies are not new independent original plants.
- The next local batch contains 12 globally reserved donor67 views across four morphologies. Native owner output: `C:/Users/USER/tomato-vlm-data-20260917/diagnostics/generated67_multihost12_native_20260918_v1`; actual running root session `50027`. Reservation: `data/sim_data/diagnostics/native848_generated67_multihost_reservation_20260918_v1/reservation_gate.json`. Do not re-reserve or relaunch; check the real owner closure and saved outputs first.
- 119 focused tests passed across coordination, identity, reservation, generator/annotation wrappers, completion and export compatibility. The shared annotation CLI reproduced the scientific results of 17 saved reference frames exactly. All 679 locked source files also match committed Git bytes. The two server generator examples ran locally, but this is not native Linux qualification.
- At Friday 22:00 KST the remaining requirement was approximately 944 accepted images/hour collectively. Current measured delivery is below that pace: the Saturday target is at risk. Increase distinct eligible view supply and accepted throughput; do not inflate counts with repeated camera templates or unresolved similarity holds.

The sections below retain earlier execution history; the Saturday goal and current snapshot above take precedence.
