"""Opt-in native848 DLSS1/2/4/8 compact lossless advancing-clock benchmark; no TRAIN admission.
...
SCHEMA = 'greenhouse.native848_fast_profile_benchmark_plan.v5'
SAMPLE = 'greenhouse.native848_fast_profile_benchmark_sample.v5'
...
DERIVATION_BINDINGS = {'D:\\research\\tomato-pi-policy\\examples\\greenhouse_sim\\sim_data\\native848_fast_profile_benchmark_v4.py': 'c2ce28ce8eeefd81106d011b20ffd1a3fa17eaeef3d98aa6b163bf103ca7bc63', 'D:\\research\\tomato-pi-policy\\data\\sim_data\\diagnostics\\native848_fast_profile_benchmark_prepare_20260916_v4\\plan.json': 'f11c5c7c47cdad0c876078447d682560d3f3a9e74a82b7a5b7006b9d551ce056', 'D:\\research\\tomato-pi-policy\\data\\sim_data\\diagnostics\\native848_fast_profile_benchmark_prepare_20260916_v4\\cpu_validation.json': '9a535dc5cfa15c345d73c756988835c6fa4306c9d7f0dbafb7f4c5bc1beeecb2'}
BUDGETS = [1, 2, 4, 8]
A, B = 'SubStem_42_view_002', 'SubStem_44_view_002'
PROFILE = 'rt2_dlss_scheduler_1over60_compact_lossless_subframe_sweep.v2'
...
PROFILE_SETTINGS = {'/rtx/rendermode':'RealTimePathTracing', '/rtx/post/aa/op':3, '/rtx/post/dlss/execMode':0,
    '/rtx-transient/post/aa/limitedOps':True, '/rtx-transient/dlssg/enabled':False,
...
def apply_profile(rep, settings):
    # Retain the previously clear DLSS AA3 denoising profile; clock is separately tested.
...
def prepare():
    plan = dict(schema=SCHEMA, resolution=[848, 408], budgets=BUDGETS,
        predecessor=deepcopy(PREDECESSOR), derivation_bindings=deepcopy(DERIVATION_BINDINGS),
        ...
        full_scene_observations=16, control_observations=20,
...
def check(plan, full=False):
    require(plan['derivation_bindings']==DERIVATION_BINDINGS, 'Changed frozen DLSS8 derivation')
    verify_bindings(DERIVATION_BINDINGS)
    ...
    require(plan['schema']==SCHEMA and plan['resolution']==[848,408] and plan['budgets']==BUDGETS
        ...
        and plan['full_scene_observations']==16 and plan['control_observations']==20
...
def save_callback_tree(folder,value):
    """Lossless pre-normalization callback, with compact typed JSON and exact arrays."""
    import json
    folder.mkdir(parents=True,exist_ok=False)
    count=0
    def encode(v):
        ...
        if isinstance(v,dict):
            if all(type(k) is str for k in v):return {'$dict':{k:encode(x) for k,x in v.items()}}
            return {'$pairs':[[encode(k),encode(x)] for k,x in v.items()]}
    ...
    with (folder/'tree.json').open('x',encoding='utf-8') as stream:
        json.dump({'schema':'greenhouse.lossless_compact_callback_tree.v1','payload':encode(value)},
            stream,separators=(',',':'),allow_nan=False)
...
def load_callback_tree(folder):
    def decode(v):
        ...
        if '$array' in v:
            name=v['$array'];require(Path(name).name==name and name.startswith('array_') and name.endswith('.npy'),'Invalid callback array path')
            return np.load(folder/name,allow_pickle=False)
        if '$object_array' in v:
            result=np.empty(len(v['$object_array']),dtype=object)
            for i,x in enumerate(v['$object_array']):result[i]=decode(x)
            return result.reshape(v['shape'])
        if '$bytes' in v:return bytes.fromhex(v['$bytes'])
        if '$dict' in v:return {k:decode(x) for k,x in v['$dict'].items()}
        if '$pairs' in v:return {decode(k):decode(x) for k,x in v['$pairs']}
        if '$tuple' in v:return tuple(decode(x) for x in v['$tuple'])
        if '$nonfinite' in v:return float(v['$nonfinite'])
        raise ValueError('Unknown compact callback token')
    envelope=read_json(folder/'tree.json')
    require(envelope['schema']=='greenhouse.lossless_compact_callback_tree.v1','Wrong callback archive schema')
...
def recording_writer(rep,unrequested_folder):
    ...
    class DiagnosticWriter(rep.Writer):
        def __init__(self):
            self.version='native848-fast-profile-benchmark-v5'
...
def full_scene(app, output, plan, scene_name, loaded):
    ...
    try:
        for item in plan['schedules'][scene_name]:
            ...
            print('FAST848_PROFILE_V5',scene_name,item['name'],receipt.get('capture_valid',False),receipt['passed_actual_automated_criteria'],flush=True)
    finally:
...
def capture(app,output,plan):
    ...
    return dict(state=STATE,scenes=scenes,controls=control,budget_status=status,
        ...
        elapsed_seconds=time.perf_counter()-started,full_scene_observations=16,
...
def audit(output,plan):
    ...
    require(request['plan_sha256']==result['plan_sha256']==sha256(request['plan_path'])
        ...
        and result['equality_used_for_acceptance'] is False and result['full_scene_observations']==16,'Changed execution scope')
    ...
    actual_files={str(p.resolve()) for p in output.rglob('*') if p.is_file() and p.name not in ('result.json','evidence_bindings.json','audit_fast_profile_v5.json')}
    ...
    try:
        ...
        require([r['name'] for r in controls_rows]==[f'b{n}_{phase}' for n in BUDGETS for phase in plan['control_order']], 'Changed twenty-control sequence')
        ...
        for scene_name,rows in result['scenes'].items():
            require([{k:r[k] for k in ('name','budget','sample_id','role')} for r in rows]==plan['schedules'][scene_name], 'Changed sixteen-frame schedule')
...
def cpu_tests():
    ...
    require(len(schedule('transition'))==12 and len(schedule('negative'))==4,'Wrong transition budget');checked+=1
    import tempfile
    from .native_instances import normalize_payload
    with tempfile.TemporaryDirectory(prefix='fast848_profile_v5_cpu_',dir=D) as temporary:
...
def main(argv=None):
    ...
    if args.action=='audit':
        ...
        require(not (output/'audit_fast_profile_v5.json').exists(),'Create-only audit required')
        write_json(output/'audit_fast_profile_v5.json',report);return
    ...
    try:
        ...
        app=SimulationApp(dict(headless=True,width=848,height=408,multi_gpu=False,renderer='RealTimePathTracing',anti_aliasing=3,
