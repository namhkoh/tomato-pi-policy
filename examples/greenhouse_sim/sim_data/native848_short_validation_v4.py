"""Opt-in native848 short-budget validation; no production or TRAIN admission.
...
SCHEMA = 'greenhouse.native848_short_validation_plan.v4'
SAMPLE = 'greenhouse.native848_short_validation_sample.v4'
STATE = 'native848_short_validation_complete_diagnostic_only'
PREDECESSOR = dict(module_path=str(Path(__file__).with_name('native848_short_validation_v3.py')),
    module_sha256='86f9b289bd9a918198bcbe9137f65bb9102048c07d3035900a29c9fc4e6639f1',
    failure_path=str(D/'native848_short_validation_trial_20260916_v3/capture/failure.json'))
BUDGETS = [1, 2, 4]
A, B = 'SubStem_42_view_002', 'SubStem_44_view_002'
PROFILE = 'reset_each_snapshot_after_authored_pose_no_hidden_settling.v4'
FLAGS = dict(training_approved=False, production_profile_qualified=False,
             ...
             geometry_novelty_qualified=False, individual_visual_review_performed=False)
SOURCES = {
...
def prepare():
    plan = dict(schema=SCHEMA, resolution=[848, 408], budgets=BUDGETS,
        predecessor=dict(**PREDECESSOR,failure_sha256=sha256(PREDECESSOR['failure_path'])),
        profile=PROFILE, sources={k:dict(path=str(p.resolve()), sha256=sha256(p)) for k,p in SOURCES.items()},
...
def check(plan, full=False):
    require(plan['predecessor']==dict(**PREDECESSOR,failure_sha256=sha256(PREDECESSOR['failure_path']))
        and sha256(PREDECESSOR['module_path'])==PREDECESSOR['module_sha256'],'Changed preserved predecessor evidence')
...
def recording_writer(rep,unrequested_folder):
    ...
    class DiagnosticWriter(rep.Writer):
        ...
        def write(self,data):
            ...
            try:
                raw=deepcopy(data)
                save_callback_tree(folder,raw)
                self.latest=normalize_payload(raw,'fast')
...
def take(rep, writer, settings, folder, steps):
    ...
    observed={k:jsonable(settings.get(k)) for k in prior.SETTINGS}
    ...
    try:
        ...
        for i,n in enumerate(steps):
            ...
            try:
                payload=step_payload(rep,writer,subframes=n)
                save_raw(folder/f'callback_{i:02d}',payload)
        ...
        require(observed=={k:jsonable(settings.get(k)) for k in prior.SETTINGS}, 'Settings changed during capture')
...
def controls(app, output, renderer):
    ...
    settings=carb.settings.get_settings();settings.set('/rtx/rendermode',renderer)
...
def full_scene(app, output, plan, scene_name, loaded):
    ...
    try:
        for item in plan['schedules'][scene_name]:
            ...
            print('SHORT848_V4',scene_name,item['name'],receipt.get('capture_valid',False),receipt['passed_actual_automated_criteria'],flush=True)
    finally:
...
def audit(output,plan):
    ...
    actual_files={str(p.resolve()) for p in output.rglob('*') if p.is_file() and p.name not in ('result.json','evidence_bindings.json','audit_v4.json')}
    ...
    try:
        ...
        workspace.finish()
    check(plan)
    return dict(state='native848_short_validation_saved_buffer_replay_complete',verified_observations=verified,**FLAGS)
...
def cpu_tests():
    ...
    import tempfile
    from .native_instances import normalize_payload
    with tempfile.TemporaryDirectory(prefix='short848_v4_cpu_',dir=D) as temporary:
        ...
        evidence=dict(requested_steps=[1],settings={},hidden_settling=False,callbacks=[event],
            effective_subframe_policy=[prior.checked_subframe_floor({},1)],
            ...
            requests=[dict(subframes=1,native_requests=1,request_index_before=0,request_index_after=1,
                callback_sequence_before=1,callback_sequence_after=2,normalized_from_callback_sequence=2)])
        write_json(folder/'request_evidence.json',evidence);verify_request(folder,[1]);checked+=1
        np.save(folder/'callback_00/rgba.npy',np.zeros_like(raw['rgb']),allow_pickle=False)
        try:verify_request(folder,[1])
        except ValueError:checked+=1
...
def main(argv=None):
    ...
    if args.action=='audit':
        ...
        require(not (output/'audit_v4.json').exists(),'Create-only audit required')
        write_json(output/'audit_v4.json',report);return
