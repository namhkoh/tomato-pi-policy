...
import numpy as np
import pytest
from .procedural_petiole_controlled_v2 import (control_for, controlled_curve,
...
def usd_fixture(tmp_path):
    ...
    root.SetCustomData({'Blender':{'generated':True}})
...
def test_authored_custom_data_does_not_include_schema_composed_defaults(tmp_path):
    from pxr import Usd
    source,output,_=usd_fixture(tmp_path)
    a,b=Usd.Stage.Open(str(source)),Usd.Stage.Open(str(output))
    old,new=a.GetDefaultPrim(),b.GetDefaultPrim()
    assert old.GetAllAuthoredMetadata()['customData']=={'Blender':{'generated':True}}
    assert old.GetMetadata('customData')!=old.GetAllAuthoredMetadata()['customData']
    assert old.GetAllAuthoredMetadata()['customData']==new.GetAllAuthoredMetadata()['customData']
    assert old.GetMetadata('customData')==new.GetMetadata('customData')
    assert verify_visible_component(source,output,spatial_changes_allowed=False)['protected_visible_attributes_exactly_preserved']


def test_authored_custom_data_tamper_is_not_hidden_by_schema_defaults(tmp_path):
    from pxr import Usd
    source,output,_=usd_fixture(tmp_path)
    stage=Usd.Stage.Open(str(output));stage.GetDefaultPrim().SetCustomData({'Blender':{'generated':False}})
    stage.GetRootLayer().Save();stage=None
    with pytest.raises(ValueError):verify_visible_component(source,output,spatial_changes_allowed=False)
