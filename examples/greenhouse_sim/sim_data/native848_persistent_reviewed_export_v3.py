"""Reviewed persistent export with separate actual annotation and visual observers.

Materialization and numerical consistency checks preserve frozen delegated v1.
The additive observer boundary authenticates the completed summary; evaluation is never
...
PINS={**PINS,
    ...
    producer.__file__:'785ef36c61f58a5c3634072ad13181144dd6ed32db2cd1e08dc53c1b26ba0ee7',
...
DELEGATION_SCHEMA='greenhouse.native848_persistent_review_export_delegation.v3'
ANNOTATION_SUMMARY_SCHEMAS={
    'greenhouse.generated19_multihost16_annotation_summary.v1',
    'greenhouse.generated67_multihost12_annotation_summary.v1',
    'greenhouse.generated41_multihost16_annotation_summary.v1'}


def validate_delegation(d,evaluation_pin,selected,review_pin,review,auth):
    """Root capture, independently named annotation observer, actual visual reviewer."""
    ...
    capture=review['actual_outer_process_completion']['capture']
    root=d['root_observed_completion']['capture'];actual=auth['native_outer_wait']
    require(set(d['root_observed_completion'])=={'capture'}
        and type(capture['tool_session']) is int and capture['tool_session']>0
        and root['tool_session']==capture['tool_session']==actual['session_id']
        and root['exit_code']==capture['exit_code']==actual['returncode']==0
        and root['wait_observed_by']==capture['wait_observed_by']=='assistant_root'
        and root['completion_chunk']==capture['completion_chunk']
        and isinstance(capture['completion_chunk'],str) and capture['completion_chunk'],
        'Only the actual successful root capture wait may be attributed to root')
    observed=review['actual_outer_process_completion']['annotation'];actual=auth['annotation_outer_wait']
    observer=d['annotation_wait_observer']
    require(isinstance(observer,str) and observer.startswith('assistant_') and observer!='assistant_root'
        and observer==observed['wait_observed_by']==auth['annotation_wait_observer']
        and type(observed['tool_session']) is int and observed['tool_session']>0
        and observed['tool_session']==actual['session_id'] and observed['exit_code']==actual['returncode']==0,
        'Root must explicitly authorize the truthful successful annotation observer')
    receipt_pin=d['annotation_wait_receipt'];summary_pin=d['annotation_summary']
    require(isinstance(receipt_pin,dict) and set(receipt_pin)=={'path','sha256'}
        and receipt_pin==auth['annotation_wait_receipt']==observed['actual_wait_receipt']
        and isinstance(summary_pin,dict) and set(summary_pin)=={'path','sha256'}
        and summary_pin==auth['annotation_summary'],
        'Delegation, authorization and review must bind one exact wait receipt and summary')
    receipt=json.loads(base.read_pin(receipt_pin).read_text())
    require(receipt['schema']=='greenhouse.actual_tool_process_wait.v1'
        and receipt['observer']==observer and receipt['session_id']==observed['tool_session']
        and receipt['exit_code']==0 and receipt['tool_chunk']==observed['completion_chunk']
        and isinstance(receipt['tool_chunk'],str) and receipt['tool_chunk']
        and receipt['result']==summary_pin and receipt['all_four_child_returncodes_zero'] is True
        and receipt['native_launch_by_this_agent'] is False,
        'Authenticated agent wait differs from attributed annotation closure')
    native=receipt['native_capture_wait']
    require(native['observer']=='root' and native['session_id']==capture['tool_session']
        and native['tool_chunk']==capture['completion_chunk'] and native['exit_code']==0,
        'Annotation wait belongs to another root capture')
    summary=json.loads(base.read_pin(summary_pin).read_text())
    public=d['persistent_batch_authority']
    require(summary['schema'] in ANNOTATION_SUMMARY_SCHEMAS
        and summary['annotation_wait_observer']==observer and summary['capture_wait_observer']=='root'
        and summary['batch_owner_complete']==d['source_owner_complete']==public['batch_owner_complete']
        and summary['batch_result']==public['batch_result']
        and summary['actual_visual_review'] is False and summary['native_launched'] is False
        and summary['export_performed'] is False and summary['accepted_training_increment']==0,
        'Pinned annotation summary has different observers or native batch authority')
    segments=_unique(summary['segments'],'segment_id')
    require(set(segments)=={'s000','s001','s002','s003'}
        and all(s['actual_child_returncode']==0 for s in segments.values())
        and summary['frames_evaluated']==sum(s['frames_evaluated'] for s in segments.values())
        and 0<summary['frames_evaluated']<=summary['expected_maximum_frames'],
        'Completed finite four-segment annotation required')
    segment=segments[public['segment_id']]
    require(segment['evaluation']==evaluation_pin
        and Path(segment['source_capture']).resolve()==Path(d['source_capture']).resolve(),
        'Selected evaluation or source capture is not the completed summary segment')
    evaluated_value=json.loads(base.read_pin(evaluation_pin).read_text())
    require(evaluated_value['schema']==EVALUATION_SCHEMA
        and evaluated_value['persistent_batch_authority']==public
        and evaluated_value['batch_owner_complete']==summary['batch_owner_complete']
        and evaluated_value['batch_capture_result']==summary['batch_result']
        and Path(evaluated_value['source_capture']).resolve()==Path(d['source_capture']).resolve()
        and evaluated_value['frames_evaluated']==segment['frames_evaluated'],
        'Selected evaluation does not authenticate the same actual segment and owner')
...
def materialize(evaluation_pin,selected_sample_ids,review_pin,output,*,authorization_pin):
    ...
    def read(spec,*,within=None):
        ...
        if within is not None:require(Path(path).is_relative_to(Path(within)),'Artifact escaped actual batch authority')
