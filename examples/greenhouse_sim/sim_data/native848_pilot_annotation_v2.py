...
from . import native848_pilot_parallel_predecessor_v1 as parallel_predecessor
...
SCHEMA='greenhouse.native848_filtered_pilot_9mm_evaluation.v2'
OWNER_SCHEMAS={
...
def authenticate_owner(trial,owner,result_sha256,read,bound,parent_metadata):
...
def run(trial,result_sha256,output,*,parent_metadata=None):
    ...
    authenticate_owner(trial,owner,result_sha256,read,bound,parent_metadata)
    ...
    for path,h in {**checker.bindings,str(Path(__file__).resolve()):sha256(__file__),str(Path(labels.__file__).resolve()):sha256(labels.__file__),str(Path(ambiguity.__file__).resolve()):AMBIGUITY_SHA256,str(Path(parallel_predecessor.__file__).resolve()):sha256(parallel_predecessor.__file__)}.items():
...
if __name__=='__main__':
    ...
    value=run(args.trial,args.result_sha256,args.output,parent_metadata=parent_metadata)
