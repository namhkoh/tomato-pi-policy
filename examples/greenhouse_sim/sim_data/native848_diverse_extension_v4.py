...
SCHEMA='greenhouse.native848_diverse_extension.v4'
REQUEST_SCHEMA='greenhouse.native848_diverse_extension_request.v4'
...
def check_group_q4_result(result,request,pins):
    version='4' if result['schema'].endswith('.v4') else '3'
...
def inspect(request_path,request_sha256):
    ...
    for spec in request['admissions']:
        ...
        group_q4=is_q4 and result['schema'].endswith(('.v3','.v4'))
        normalized=result
        if is_q4:
        ...
        parallel=normalized['schema']=='greenhouse.native848_diverse_bulk_q3_adapter.v4'
        if parallel:
            parallel_helper=group_api_for('greenhouse.native848_diverse_group_review.v4',request,pins)
            parallel_helper.verify_adapter4(normalized,pins)
        ...
        if group_mode:
            ...
            if group_q4:
                minimum_schema=('greenhouse.native848_diverse_group_review.v4' if result['schema'].endswith('.v4') else
                                'greenhouse.native848_diverse_group_review.v3')
                require(group_review['schema'] in (minimum_schema,'greenhouse.native848_diverse_group_review.v4'),
