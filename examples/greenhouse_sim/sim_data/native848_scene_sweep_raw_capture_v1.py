"""One fixed greenhouse; numeric proposals, native geometry/workspace checks once, annotation offline."""
...
MODULE='sim_data.native848_scene_sweep_raw_capture_v1'
REQUEST_SCHEMA='greenhouse.native848_scene_sweep_raw_request.v1'
RESULT_SCHEMA='greenhouse.native848_scene_sweep_raw_capture.v1'
OBSERVATION_SCHEMA='greenhouse.native848_scene_sweep_raw_observation.v1'
CONTEXT_SCHEMA='greenhouse.native848_scene_sweep_raw_context.v1'
...
def implementation_bindings():
    names=('native848_scene_sweep_capture_v1.py','native848_bulk_workspace_v1.py','native848_original_direct_worker_v2.py','static_geometry_parts_cache_v1.py','native848_all_petiole_9mm_v2.py','static_guard.py','native848_bulk_io_v1.py')
...
def check_request(req):
    ...
    require(proof['proposal_only'] is True and proof['scene_collision_pending'] is True
        and proof['native_floor_and_workspace_validation_required'] is True
        and proof['records']==req['records'] and proof['expected_census']==req['expected_census'],
        'Exact numeric proposal proof with explicit native scene checks pending required')
...
def validate_result(result,request,output):
    ...
    for hold in result['holds']:
        name=hold['sample_id'];require(name in scheduled and name not in held
            and ((hold['reason']=='whole_robot_scene_collision' and hold['screen']['passed'] is False)
                 or (hold['reason']=='9mm_workspace' and hold['workspace']['result']['workspace_passed'] is False)), 'Foreign, repeated or unsupported hold')
        if hold['reason']=='9mm_workspace':
            w=hold['workspace'];r=scheduled[name];require(w['schema']=='greenhouse.native848_bulk_workspace.v1' and w['target_id']==r['target_id'] and w['per_frame_camera_FK_verified'] is True and np.allclose(w['nominal_world_m'],r['pilot_9mm_geometry']['nominal']['world_m'],atol=1e-9,rtol=0),'Workspace hold belongs to another pose target')
        held.append(name)
    ...
    for ordinal,(receipt,name) in enumerate(zip(result['frames'],expected_names),warmup_count+1):
        ...
        w=observation['actual_workspace_9mm'];require(w['schema']=='greenhouse.native848_bulk_workspace.v1' and w['target_id']==observation['target_id'] and w['per_frame_camera_FK_verified'] is True and w['per_frame_cached_solution_FK_verified'] is True and np.allclose(w['nominal_world_m'],observation['geometry']['nominal']['world_m'],atol=1e-9,rtol=0),'Native workspace proof belongs to another target or point')
        require(w['result']['workspace_passed'] is True
            and pose['source_body_orientation_preserved'] is False
            and pose['pose_sampling']=='new_body_orbit_and_head_fk_v1','Actual native workspace/new-pose provenance required')
    stats=result['workspace_stats'];require(stats['checked_frames']==len(result['frames'])+sum(h['reason']=='9mm_workspace' for h in result['holds']),'Workspace frame/hold count differs')
    require(all(context['source_bindings'].get(p)==h for p,h in stats['source_bindings'].items()),'Workspace sources absent from native closure');verify_bindings(stats['source_bindings'])
    return context
...
def capture(app,req,checked,output,request_pin):
    ...
    (output/'holds').mkdir(exist_ok=False)
    ...
    try:
        with ExitStack() as cleanup:
            ...
            bindings.update(workspace.bindings)
            ...
            for record in records:
                ...
                if not screen['passed']:
                    holds.append(dict(sample_id=record['sample_id'],reason='whole_robot_scene_collision',screen=screen));save_json(output/'holds'/('hold_'+record['sample_id']+'.json'),holds[-1]);continue
                pose['visual_bound_screen']=screen;pose['whole_robot_collision_checked']=True
                pose.update(source_body_orientation_preserved=False,pose_sampling='new_body_orbit_and_head_fk_v1')
                ...
                if not reach['result']['workspace_passed']:
                    holds.append(dict(sample_id=record['sample_id'],reason='9mm_workspace',workspace=reach));save_json(output/'holds'/('hold_'+record['sample_id']+'.json'),holds[-1]);continue
                if monitor is None:
                    ...
                    context=dict(schema=CONTEXT_SCHEMA,request=request_pin,native_identity=pin(output/'native_identity.json'),source_bindings=bindings,scene_anchor=req['scene_anchor'],profile=req['profile'],renderer_settings=render_settings,
                        ...
                        task_input_policy=dict(query_input=False,clean_full_rgb=True,aligned_depth=True),camera_transition_motion_validated=False,static_snapshots_only=True,proposal_stage_scene_collision_pending=True,native_collision_and_workspace_validated_before_each_saved_frame=True,**FLAGS)
                ...
                observation=dict(schema=OBSERVATION_SCHEMA,observation_id=name,sample_id=name,context=context_pin,source_family=record['source_family'],target_variant=record['target_variant'],plant_root=record['plant_root'],
                    ...
                    calibration=cal,robot_snapshot=pose,actual_workspace_9mm=reach,geometry=geo,mapping=pin(mp),rendered_camera_params=jsonable(payload['camera_params']),request_evidence=e,
            ...
            workspace_stats=workspace.finish();verify_bindings(workspace_stats['source_bindings'])
            ...
            result=dict(schema=RESULT_SCHEMA,request=request_pin,context=context_pin,native_identity=pin(output/'native_identity.json'),frames=receipts,holds=holds,requests=requests,warmup_requests=warmups,
                workspace_stats=workspace_stats,request_count=writer.request_index,callback_count=writer.sequence,stage_count=1,render_product_count=1,scene_assets_unchanged=True,timings=timings,**FLAGS)
