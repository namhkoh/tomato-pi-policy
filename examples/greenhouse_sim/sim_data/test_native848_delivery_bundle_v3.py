...
from . import native848_delivery_bundle_v2 as prior
from . import native848_delivery_bundle_v3 as bundle

DIAG=Path('D:/research/tomato-pi-policy/data/sim_data/diagnostics/native848_delivery_bundle_v3_CPU_20260918_v1')
...
class AdapterChecks(unittest.TestCase):
    ...
    def scene_component(self,change=None):
        spec,r=self.component()
        r.update(schema=bundle.sweep.FRAME_SCHEMA,source_geometry_modified=False,new_independent_source_family=False)
        ev=dict(path='fixture_evaluation',sha256='a'*64);rv=dict(path='fixture_review',sha256='b'*64)
        r['source_provenance']=dict(evaluation=ev,review=rv,capture_kind='greenhouse.native848_scene_sweep_context.v1')
        if change:change(r)
        index=self.root/'index.jsonl';index.write_text(json.dumps(r)+'\n')
        m=json.loads((self.root/'manifest.json').read_text())
        m.update(schema=bundle.sweep.SCHEMA,index=bundle.pin(index),implementation_sha256=bundle.LOADER_PINS['scene_sweep'],
            source_evaluation=ev,actual_review=rv,target_hint_used_to_select_answer=False,input_query=False)
        manifest=self.root/'manifest.json';manifest.write_text(json.dumps(m))
        result=self.root/'result.json';result.write_text(json.dumps(dict(schema=bundle.sweep.SCHEMA,state=bundle.sweep.STATE,manifest=bundle.pin(manifest),unique_images=1)))
        spec.update(kind='scene_sweep',result=bundle.pin(result),manifest=bundle.pin(manifest),index=bundle.pin(index))
        return spec,r

    def test_sweep_counts_original_donor_without_new_geometry(self):
        spec,r=self.scene_component();root,rows=bundle._component(spec)
        e=bundle.entry_for('sweep','scene_sweep',0,r)
        self.assertFalse(e['generated_geometry']);self.assertFalse(e['new_independent_source_family'])
        self.assertEqual(e['geometry_source_id'],r['source_family']);self.assertEqual(rows,[r])

    def test_sweep_reassigned_capture_rejected(self):
        spec,r=self.scene_component(lambda r:r['source_provenance'].update(capture_kind='old_context'))
        with self.assertRaises(ValueError):bundle._component(spec)

    def test_sweep_wrong_split_rejected(self):
        spec,r=self.scene_component(lambda r:r.update(split='test'))
        with self.assertRaises(ValueError):bundle._component(spec)

    def test_sweep_generated_geometry_claim_rejected(self):
        spec,r=self.scene_component(lambda r:r.update(source_geometry_modified=True))
        with self.assertRaises(ValueError):bundle._component(spec)
