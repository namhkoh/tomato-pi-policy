...
SCHEMA = 'greenhouse.native848_short_validation_plan.v5'
SAMPLE = 'greenhouse.native848_short_validation_sample.v5'
...
PROFILE = 'reset_each_snapshot_after_authored_pose_no_hidden_settling.v4'  # Renderer/reset profile unchanged.
...
def prepare():
    plan = dict(schema=SCHEMA, resolution=[848, 408], budgets=BUDGETS,
        ...
        full_scene_observations=8, control_observations=10,
...
def check(plan, full=False):
    ...
    require(plan['schema']==SCHEMA and plan['resolution']==[848,408] and plan['budgets']==BUDGETS
        ...
        and plan['full_scene_observations']==8 and plan['control_observations']==10
...
def recording_writer(rep,unrequested_folder):
    ...
    class DiagnosticWriter(rep.Writer):
        def __init__(self):
            self.version='native848-short-validation-v5'
...
def full_scene(app, output, plan, scene_name, loaded):
    ...
    try:
        for item in plan['schedules'][scene_name]:
            ...
            print('SHORT848_V5',scene_name,item['name'],receipt.get('capture_valid',False),receipt['passed_actual_automated_criteria'],flush=True)
    finally:
...
def capture(app,output,plan):
    ...
    return dict(state=STATE,scenes=scenes,controls=control,budget_status=status,
        ...
        elapsed_seconds=time.perf_counter()-started,full_scene_observations=8,
...
def audit(output,plan):
    ...
    require(request['plan_sha256']==result['plan_sha256']==sha256(request['plan_path'])
        ...
        and result['equality_used_for_acceptance'] is False and result['full_scene_observations']==8,'Changed execution scope')
    ...
    actual_files={str(p.resolve()) for p in output.rglob('*') if p.is_file() and p.name not in ('result.json','evidence_bindings.json','audit_v5.json')}
    ...
    try:
        ...
        require([r['name'] for r in controls_rows]==[f'b{n}_{phase}' for n in BUDGETS for phase in plan['control_order']], 'Changed 10-control sequence')
        ...
        for scene_name,rows in result['scenes'].items():
            require([{k:r[k] for k in ('name','budget','sample_id','role')} for r in rows]==plan['schedules'][scene_name], 'Changed 8-frame schedule')
...
def cpu_tests():
    ...
    require(len(schedule('transition'))==6 and len(schedule('negative'))==2,'Wrong transition budget');checked+=1
    import tempfile
    from .native_instances import normalize_payload
    with tempfile.TemporaryDirectory(prefix='short848_v5_cpu_',dir=D) as temporary:
        ...
        evidence=dict(requested_steps=[BUDGETS[0]],settings={},hidden_settling=False,callbacks=[event],
            effective_subframe_policy=[prior.checked_subframe_floor({},BUDGETS[0])],
...
def main(argv=None):
    ...
    if args.action=='audit':
        ...
        require(not (output/'audit_v5.json').exists(),'Create-only audit required')
        write_json(output/'audit_v5.json',report);return
