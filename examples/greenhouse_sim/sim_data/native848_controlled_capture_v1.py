"""Capture up to two mounted-camera views of one generated full greenhouse plant.
...
MODULE = 'sim_data.native848_controlled_capture_v1'
REQUEST_SCHEMA = 'greenhouse.controlled_capture_request.v1'
RESULT_SCHEMA = 'greenhouse.controlled_capture_result.v1'
...
def implementation_bindings():
    names = ['native848_generated_morphology_scene_v1.py', 'native848_morphology_diagnostic_v1.py',
             ...
             'prepare_controlled_views_v1.py', 'native848_bulk_workspace_v1.py']
...
def check_request(request):
    require(request['schema']==REQUEST_SCHEMA and type(request['max_frames']) is int
            and request['max_frames']==2 and request['training_approved'] is False,
    ...
    require(candidates['schema']=='greenhouse.controlled_mounted_camera_candidates.v1'
            and 1<=len(candidates['records'])<=3, 'One to three fixed camera proposals required')
    verify_bindings(candidates['source_bindings'])
    require(len({r['sample_id'] for r in candidates['records']})==len(candidates['records']), 'Repeated camera ID')
...
def capture(app, request, candidates, output, *, request_pin):
    ...
    from . import native848_generated_morphology_scene_v1 as generated
    ...
    bindings={**candidates['source_bindings'],**population['source_bindings'],**source_hashes(stage),**request['implementation_bindings']}
    ...
    context=dict(schema='greenhouse.native848_generated_population_context.v1',dataset_split='train',
        ...
        renderer_settings=render_settings,**FLAGS)
    ...
    with ExitStack() as cleanup:
        ...
        sink=FrameSink(output/'frames',max_frames=2,max_bytes=64*2**20,workers=1);cleanup.callback(sink.close)
        warmed=False;previous=None;monitor=None;frozen_sources=None
        for record in records:
            if len(committed)>=2:break
            ...
            observation=dict(schema='greenhouse.controlled_native_observation.v1',observation_id=sample_id,sample_id=sample_id,
        ...
        receipts=sink.close()
        require(len(receipts)==len(committed)<=2 and writer.capture_error is None, 'Capture callback/write mismatch')
        if not warmed:
            frozen_sources=source_hashes(stage)
            for path,h in frozen_sources.items():
...
def owned_run(args):
    ...
    with owner_lock():
        ...
        require(result['schema']==RESULT_SCHEMA and 0<=len(result['frames'])<=2
