...
import numpy as np
from . import native848_controlled_pair_plan_v2 as ap
...
def capture(app, output, plan, *, profile_geometry_cache=False):
    ...
    from .native848_controlled_scene_v2 import prepare_native_scene
    ...
    # prepare_native_scene freshly applies daylight and requires exact equality
    # with this manifest value before returning the reconstructed scene.
    lighting = old_manifest["lighting"]
...
def main(argv=None):
    ...
    # Pure hash/schema checks only until SimulationApp selects its own USD ABI.
    require(plan['schema_version']==ap.SCHEMA and plan['resolution']==[848,408]
        and all(plan[k] is v for k,v in ap.FLAGS.items()), 'Changed controlled pair preflight scope')
    ...
    try:
        ...
        # Full geometry/catalogue replay occurs only after native USD initialization.
        ...
        write_json(output/'failure.json',dict(state='source_frame_controlled_native848_pair_failed',error=traceback.format_exc(),training_approved=False))
        raise
    finally:
