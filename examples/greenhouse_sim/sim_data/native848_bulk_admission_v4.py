...
class Admission:
    def __init__(self, context_path, context_sha256, prior_path, prior_sha256, output):
        ...
        write_json(self.output/'request.json', dict(context_path=str(self.context_path),
            ...
            consumer_sha256=sha256(__file__), annotation_epoch=queries.ANNOTATION_EPOCH,
            query_selector_sha256=sha256(queries.__file__),
            query_selector_dependencies={str(Path(queries.previous.__file__).resolve()):sha256(queries.previous.__file__)},
    ...
    def frame(self, observation_path):
        ...
        label, trace, selection = queries.annotate_v3(meta,target['anatomy_report'],rgb,depth,valid,
            components,self.catalogue,target_mask=mask.astype(np.uint8)*255,
            renderer_ids=ids,renderer_mapping=mapping,expected_label=baseline)
        ...
        decision = ('exclude_duplicate_or_prior_hold' if duplicate else 'exclude_local_clarity' if not local
            else 'hold_query_trace_or_route_separation' if not traced else 'hold_workspace' if not reachable
    ...
    def finish(self):
        ...
        result = dict(schema='greenhouse.original848_bulk_admission.v4', annotation_epoch=queries.ANNOTATION_EPOCH,
