...
SCHEMA='greenhouse.native848_filtered_pilot_9mm_evaluation.v1'
OWNER_SCHEMA='greenhouse.native848_unique9mm_pilot_owner.v1'
...
def run(trial,result_sha256,output):
    ...
    require(owner['schema']==OWNER_SCHEMA and owner['state']=='owned_unique9mm_filtered_capture_exited_pending_census_annotation_and_visual_review'
            and owner['complete_9mm_annotation'] is False and owner['training_approved'] is False,'Actual capture-only owner result required')
    complete=read(trial/'owner_complete.json',sha256(trial/'owner_complete.json'))
    require(complete['result_sha256']==result_sha256 and complete['capture_only'] is True,'Whole owner completion required')
    ...
    for path,h in {**checker.bindings,str(Path(__file__).resolve()):sha256(__file__),str(Path(labels.__file__).resolve()):sha256(labels.__file__)}.items():
    ...
    result=dict(schema=SCHEMA,owner_result=dict(path=str(trial/'result.json'),sha256=result_sha256),
        ...
        workspace_stats=stats,implementation=dict(path=str(Path(__file__).resolve()),sha256=sha256(__file__)),
...
if __name__=='__main__':
    ...
    args=parser.parse_args();value=run(args.trial,args.result_sha256,args.output)
