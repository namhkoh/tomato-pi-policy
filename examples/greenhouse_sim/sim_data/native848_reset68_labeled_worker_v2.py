...
def native_command(args):
    return ['D:/isaac-sim-6.0.1/python.bat','-B','-u','-m','sim_data.native848_reset68_labeled_worker_v2',
...
def record_native_identity(args):
    ...
    return dict(schema='greenhouse.native848_reset68_labeled_worker_identity.v2',owner_identity=owner,
...
def validate_native_identity(value,command,*,owner_identity=None):
    require(value['schema']=='greenhouse.native848_reset68_labeled_worker_identity.v2'
        and value['same_snapshot_identity_checks'] is True and value['command']==command
        and command[1:5]==['-B','-u','-m','sim_data.native848_reset68_labeled_worker_v2'],
...
def runtime_evidence(phase,config,native_pin):
    ...
    return dict(schema='greenhouse.native848_reset68_labeled_runtime_evidence.v2',phase=phase,
...
def validate_optout_capture(capture,value,context):
    ...
    for phase in ('start','end'):
        ...
        require(evidence['schema']=='greenhouse.native848_reset68_labeled_runtime_evidence.v2'
