"""Finite persistent native capture with one stage/product across compatible foregrounds.
...
MODULE='sim_data.native848_persistent_capture_v1'
REQUEST_SCHEMA='greenhouse.native848_persistent_capture_request.v1'
RESULT_SCHEMA='greenhouse.native848_persistent_batch_capture.v1'
SEGMENT_SCHEMA='greenhouse.native848_persistent_segment_capture.v1'
OBSERVATION_SCHEMA='greenhouse.native848_persistent_native_observation.v1'
...
def implementation_bindings():
    names=('native848_persistent_foreground_v1.py','native848_controlled_capture_v2.py')
...
def check_request(request):
    require(request['schema']==REQUEST_SCHEMA and request['implementation_bindings']==implementation_bindings(),
        'Exact persistent capture implementation required')
    verify_bindings(request['implementation_bindings'])
    require(request['training_approved'] is False and isinstance(request['segments'],list)
        and 1<=len(request['segments'])<=16,'Finite unapproved segment schedule required')
    segments=[];keys=[];total=0;ids=set()
    for i,s in enumerate(request['segments']):
        require(set(s)=={'segment_id','source_request'} and s['segment_id']==f's{i:03d}'
            and s['segment_id'] not in ids,'Canonical unique ordered segment identity required')
        ids.add(s['segment_id']);source=bound(s['source_request']);candidates=baseline.check_request(source)
        total+=source['max_frames'];keys.append(compatible_key(candidates));segments.append((source,candidates))
    require(type(request['max_frames']) is int and total==request['max_frames'] and 1<=total<=1024,
        'Bounded exact total frame schedule required')
    require(all(k==keys[0] for k in keys) and request['compatibility']==keys[0],
        'Only exact same donor/slot/scene/profile groups may share this stage')
    return segments


def make_request(source_requests):
    require(isinstance(source_requests,list) and 1<=len(source_requests)<=16,'One-to-sixteen pinned source requests required')
    segments=[dict(segment_id=f's{i:03d}',source_request=spec) for i,spec in enumerate(source_requests)]
    sources=[bound(s) for s in source_requests];candidates=[baseline.check_request(s) for s in sources]
    value=dict(schema=REQUEST_SCHEMA,segments=segments,max_frames=sum(s['max_frames'] for s in sources),
...
def validate_batch_registry(result, request, capture_root, *, request_pin, native_identity):
    ...
    for scheduled,registered in zip(request['segments'],result['segments']):
        ...
        require(0<=len(value['frames'])<=source['max_frames']
        ...
        for frame in value['frames']:
            ...
            authority=bound(observation['context'])['persistent_batch_authority']
...
def capture_segment(app, request, candidates, output, *, source_request_pin, batch_request_pin, segment_id, session, writer, batch_native_identity, previous, anchor_cache):
    import omni.usd
    ...
    with ExitStack() as cleanup:
        ...
        save_json(output/'result.json',dict(schema=SEGMENT_SCHEMA,state='captured_pending_complete_annotation_and_visual_review',
            ...
            morphology_id=morphology,original_143_backgrounds_preserved=True,**FLAGS))
...
def capture(app,request,segments,output,*,request_pin):
    ...
    with ExitStack() as cleanup:
        ...
        for scheduled,(source,candidates) in zip(request['segments'],segments):
            ...
            result,previous=capture_segment(app,source,candidates,target,
        ...
        result=dict(schema=RESULT_SCHEMA,state='captured_pending_complete_annotation_and_visual_review',
            ...
            total_capture_seconds=time.perf_counter()-started,background_closure=background_closure,**FLAGS)
