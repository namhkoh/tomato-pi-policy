...
IMPLEMENTATIONS={'greenhouse.native848_pilot_parallel.v1':'v1','greenhouse.native848_pilot_parallel.v2':'v2','greenhouse.native848_pilot_parallel.v3':'v3'}
...
def validate_retirement_evidence(proof,slot,coordinator,owned):
    require(proof['slot']==slot['slot'] and proof['command_identity']==slot['command_identity']
        ...
        and 0<proof['bounded_retirement_wait_seconds']<=10 and proof['process_control_performed'] is False,
...
def check_parallel(value,*,rows=None):
    ...
    for slot in slots:
        ...
        if version=='v3':
            require(arg(launch['command'],'-m')=='sim_data.native848_pilot_sibling_worker_v3','Parallel3 native worker module differs')
            ...
            auxiliaries=validate_retirement_evidence(retirement,slot,coordinator,owned)
    ...
    return dict(metadata=metadata,coordinator_identity=coordinator,source_bindings=bindings,
        completed_capture_slots=sorted(results),skipped_slots=sorted(skipped),all_recorded_native_children_absent=True,all_recorded_auxiliary_identities_absent=(version=='v3'),
