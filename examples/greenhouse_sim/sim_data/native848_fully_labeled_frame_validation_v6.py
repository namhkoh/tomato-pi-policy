...
AMBIGUITY_SHA256='2f9936a03bf2b405dd97fac19988afcbb4abe13612055c65c13098291d44754d'
...
from . import native848_pilot_annotation_v16 as full_annotation
from . import native848_fully_labeled_worker_v1 as full_worker
from . import native848_fully_labeled_scene_v1 as full_scene
ANNOTATION_SHA='c3b709eba54f0ca65e23251165ea1021d4c203019369ee9aad3f089e756b2fc3'
...
def validate_joint_identity(target,entry,catalogue,annotation_bindings,native_bindings):
    """Reconstruct the exact same-instance joint proof, including its absence."""
    require(target['annotation_epoch']==EPOCH,'Mixed target annotation epochs')
    evaluated=any(key in target for key in ('attachment_joint_ownership','target_bbox_xyxy',
        'visibility','junction_continuity','local_clarity','parent_context','workspace'))
    if not evaluated:
        require(target.get('automated_pass') is not True,'Candidate lacks joint evaluation evidence')
        return
    require(target['target_id']==entry['target_id'] and target['source_family']==entry['source_family'],
        'Joint source target differs from native instance')
    additional,proof=full_annotation.labels.joint_ownership.verified_additional_parent(entry,catalogue)
    require('attachment_joint_ownership' in target and target['attachment_joint_ownership']==proof,
        'Saved attachment joint ownership differs from authenticated source geometry')
    if proof is not None:
        require(all(annotation_bindings.get(p)==h and native_bindings.get(p)==h
            for p,h in proof['source_bindings'].items()),
            'Joint geometry is not bound by both annotation and original native CPU sources')
    instance,component_id=entry['target_id'].split('/')
    declared_id=entry['report']['components'][component_id]['parent']
    parents=[r for r in catalogue if r['variant_id']==instance and r['component_id']==declared_id]
    require(len(parents)==1 and parents[0]['organ_type']=='main_stem'
        and parents[0]['source_plant_id']==entry['source_family'],'Exact same-instance declared parent required')
    expected_indices=sorted({parents[0]['component_index']}|additional)
    if 'parent_context' in target:
        parent=target['parent_context']
        require(parent['parent_component_indices']==expected_indices
            and parent['parent_scope']=='declared_parent_and_authenticated_joint_forming_segment',
            'Parent clearance did not use the complete authenticated joint union')
...
def fully_labeled_identity(annotation):
    ...
    joint=full_annotation.labels.joint_ownership
    joint_sha='37f7b121202a3e04c45166ab47c8e976bc7d4c74cfa546d1ac1ae65bf4d2f6aa'
    require(digest(joint.__file__)==joint_sha and annotation['source_bindings'].get(str(Path(joint.__file__).resolve()))==joint_sha,
        'Frozen joint ownership implementation provenance required')
    ...
    for t in annotation['targets']:
        ...
        validate_joint_identity(t,entry,by_variant[row['variant_id']],annotation['source_bindings'],cpu['source_bindings'])
