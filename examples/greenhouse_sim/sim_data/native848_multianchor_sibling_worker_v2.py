"""Admission-only wrapper for two to four explicitly declared owned balanced multi-anchor workers.
...
def main(argv=None):
    ...
    require(ticket['schema']=='greenhouse.native848_multianchor_parallel_ticket.v2' and type(ticket['worker_count']) is int and ticket['worker_count'] in (2,3,4)
        and type(ticket['slot']) is int and 0<=ticket['slot']<ticket['worker_count'] and type(ticket['frames_per_worker']) is int and 2<=ticket['frames_per_worker']<=512
    ...
    try:
        ...
        plan=read(plan_path);require(plan['schema']==api.SCHEMA and plan['max_frames']==ticket['frames_per_worker']
            and len(plan['schedule'])==plan['max_frames'],'Actual bounded multi-anchor schedule differs')
        output.mkdir()
