"""Admission-only wrapper for exactly two owned compact bulk workers.
...
from . import native848_bulk_sibling_gate_v1 as gate
...
def main(argv=None):
    ...
    try:
        ...
        require(release['ticket_sha256']==args.ticket_sha256 and release['slot']==ticket['slot']
            and release['coordinator_identity']==coordinator,'Changed release identity')
        ...
        plan=read(plan_path);require(plan['max_frames']==32,'First parallel probe is exactly32frames perworker')
        output.mkdir()
        ...
        save_json(destination/'failure.json',dict(state='owned_original848_parallel_worker_failed',
            error=traceback.format_exc(),automatic_retries=False,training_approved=False,
