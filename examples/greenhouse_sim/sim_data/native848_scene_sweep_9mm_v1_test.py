...
from . import native848_scene_sweep_9mm_v1 as api
...
def ledger():
    ...
    return result,[dict(sample_id=name) for name in ('a','held','b')],obs,profile
...
class OwnerTests(unittest.TestCase):
    def run_case(self,mutation=None):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            context=dict(schema=api.producer.CONTEXT_SCHEMA,request=rp,native_identity=ip,profile=profilepin,scene_anchor=anchor)
            cp=save(capture/'context.json',context);receipts=[]
