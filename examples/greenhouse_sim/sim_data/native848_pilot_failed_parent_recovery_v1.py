"""Finite read-only recovery of two completed slots from one failed pilot parent.

This does not turn the failed parent into a successful run or approve an image.
Only the exact 51/22 completed schedules can proceed to new annotation and QA.
"""
...
PARENT = Path('C:/Users/USER/tomato-vlm-data-20260917/diagnostics/native848_unique9mm_pilot_parallel_20260917_v1/four_families_103_candidates_v1')
LAUNCH = ROOT/'data/sim_data/diagnostics/native848_unique9mm_pilot_parallel_launch_20260917_v1/four_families_103_candidates_v1'
OWNER = H/'run_native848_pilot_parallel_v1.py'
OWNER_SHA = '506265ad53b3fd1ea2a254b96fc86d652eb419af2a3eb2331b8eb452a09fa477'
WORKER = Path(__file__).with_name('native848_pilot_sibling_worker_v1.py')
WORKER_SHA = 'dc6752d59aba9db6503465ec9fbcaa833d0ac0f4eec0fb219e4b6683db5b35fb'
RESULT_SHA = '65a380304fd22e2e79a48519c50addbfd620e636843dd6df99a8a51f73b5555a'
METADATA_SHA = '6b291d6a190d1664a4253c07528c4b55894ef727316c1c533ac6b048ec97cec7'
SCHEMA = 'greenhouse.native848_pilot_failed_parent_recovery.v1'
RECOVERABLE = [0, 1]
EXPECTED_COUNTS = [51, 22, 20, 10]
...
def _validate(*, rows=None, require_idle=False):
    ...
    require(Path(metadata['trial_path']).resolve() == PARENT and result['schema'] == 'greenhouse.native848_pilot_parallel.v1'
            and result['requested_worker_count'] == 4 and result['frame_counts'] == EXPECTED_COUNTS
            and result['planned_frames'] == 103 and result['committed_frames'] == 73
    ...
    require(failure['result']['sha256'] == RESULT_SHA and failure['incomplete_slots'] == result['incomplete_slots']
            and sorted(r['slot'] for r in result['incomplete_slots']) == [2, 3]
    ...
    require(intent['coordinator_identity'] == coordinator and intent['native_workers'] == request['worker_count'] == 4
            ...
            and intent['planned_frames'] == 103, 'Actual parent intent/request scope differs')
    ...
    require(len(result['completed_slots']) == len(slots) == 4 and set(slots) == {0,1,2,3}
            and len(result['results']) == 2 and set(successful) == {0,1}, 'Exact all-child and successful-slot inventories required')
    ...
    for index in range(4):
        ...
        require(child_exit['returncode'] == [0,0,0,1][index]
        ...
        require(ready['slot'] == index and ready['native_identity'] == native and ready['command_identity'] == command
                ...
                and ticket['schema'] == 'greenhouse.native848_pilot_parallel_ticket.v1'
        ...
        e.obj(folder/'command_identity.json')
        ...
        if index not in RECOVERABLE:
            ...
            if index == 3:
                fail = e.obj(folder/'failure.json')
                require(fail['simulation_app_construction_started'] is False
                        and fail['exception_type'] == 'ValueError'
                        and 'Owned command/native/coordinator process disappeared before admission' in fail['error']
                        and fail['ticket_sha256'] == sha256(folder/'ticket.json')
                        and not (folder/'simulation_app_starting.json').exists()
                        and not (folder/'capture/batch_context.json').exists(), 'Actual sibling-admission failure receipt required')
            else:
                e.obj(folder/'capture/batch_manifest.json'); e.obj(folder/'capture/result.json')
            continue
    ...
    return dict(metadata=metadata,coordinator_identity=coordinator,parent_failed=True,parent_returncode=1,
            recoverable_slots=RECOVERABLE,excluded_slots=[2,3],completed_raw_frames=73,
...
def check(spec,rows=None):
    ...
    require(audit['schema'] == SCHEMA and audit['implementation'] == pin(__file__)
            ...
            and audit['recoverable_slots'] == RECOVERABLE and audit['excluded_slots'] == [2,3]
    ...
    for key in ('metadata','outer_exit','outer_launch','parent_result','parent_failure','slot_proofs','source_bindings',
                'successful_slot_results','parent_returncode','completed_raw_frames'):
