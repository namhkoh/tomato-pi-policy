...
def audit_capture(capture, *, plan_path, plan_sha256, result_sha256):
    ...
    for index, name in enumerate(expected_ids):
        ...
        require(direct == dict(pose_cache_sha256=plan['cache_sha256'], cached_sample_id=name,
            anchor_pair_plan_sha256=rec['anchor_pair_plan_sha256'], runtime_pose_search=False,
        ...
        require(catalogue == expected_catalogue(anchor, 'original_control', reports, None)
        ...
        require(sup['target_id'] == row['target_id'] and sup['source_target_id'] == sup['conservative_view_cap_group'] == row['target_id']
        ...
        generated_ids = [i for i, path in mapping.items() if path.startswith('/World/GeneratedNativePilot/')]
        require(meta['original_geometry_only'] is True and meta['generated_plant_native_pixels']
            == int(np.isin(ids, generated_ids).sum()) == 0, 'Generated geometry entered original capture')
        records.append(dict(sample_id=name, target_id=row['target_id'], source_target=row['target_id'],
            sample_path=str(mp), sample_sha256=sha256(mp), rgb_sha256=sha256(folder/'inputs/rgb.png'),
            anchor_pair_plan_path=rec['anchor_pair_plan_path'], anchor_pair_plan_sha256=rec['anchor_pair_plan_sha256'],
    ...
    return dict(schema='greenhouse.original848_direct_buffer_geometry_audit.v1',
        state='original848_direct_buffers_geometry_replayed_pending_clarity_workspace_review',
        ...
        strict_clarity_qualified=False, original_workspace_qualified=False,
