"""Root-delegated generated export with root capture and attributed annotation waits.
...
DELEGATION_SCHEMA='greenhouse.native848_generated_review_export_delegation.v2'
...
def validate_delegation(d,evaluation_pin,selected,review_pin,review,auth):
    ...
    observed=review['actual_outer_process_completion']['capture']
    root=d['root_observed_completion']['capture'];actual=auth['native_outer_wait']
    require(root['tool_session']==observed['tool_session']==actual['session_id']
        and root['exit_code']==observed['exit_code']==actual['returncode']==0
        and root['wait_observed_by']==observed['wait_observed_by']=='assistant_root',
        'Root-observed successful native closure required')
    observed=review['actual_outer_process_completion']['annotation'];actual=auth['annotation_outer_wait']
    require(type(observed['tool_session']) is int and observed['tool_session']>0
        and observed['tool_session']==actual['session_id'] and observed['exit_code']==actual['returncode']==0,
        'Successful matching annotation closure required')
    if observed['wait_observed_by']=='assistant_root':
        root=d['root_observed_completion']['annotation']
        require(root['tool_session']==observed['tool_session'] and root['exit_code']==0
            and root['wait_observed_by']=='assistant_root','Root annotation closure differs')
    else:
        require(observed['wait_observed_by']==review['reviewer']!='assistant_root',
            'Annotation observer must be the attributed reviewer')
        receipt_pin=d.get('annotation_wait_receipt')
        require(isinstance(receipt_pin,dict) and set(receipt_pin)=={'path','sha256'}
            and receipt_pin==auth.get('annotation_wait_receipt')==observed.get('actual_wait_receipt'),
            'Root delegation, authorization and review must bind the same agent wait receipt')
        receipt=json.loads(base.read_pin(receipt_pin).read_text())
        require(receipt['schema']=='greenhouse.native848_generated_agent_annotation_wait.v1'
            and receipt['role']=='annotation' and receipt['actual_wait_observed'] is True
            and receipt['wait_observed_by']==observed['wait_observed_by']
            and receipt['tool_session']==observed['tool_session'] and receipt['exit_code']==0
            and receipt['evaluation']==evaluation_pin
            and receipt['source_owner_complete']==d['source_owner_complete']
            and Path(receipt['source_capture']).resolve()==Path(d['source_capture']).resolve(),
            'Pinned agent wait must belong to this successful evaluation and native owner')
