...
Every per-frame gate stays unchanged. New post3 Q4 rows use predeclared margin
flags; older Q4 rows remain individual. Existing receipts and labels stay unchanged.
"""
...
SCHEMA = 'greenhouse.native848_diverse_group_inventory.v5'
REVIEW_SCHEMA = 'greenhouse.native848_diverse_group_review.v5'
PROTOCOL_SCHEMA = 'greenhouse.native848_diverse_group_protocol.v2'
...
PROTOCOL = dict(schema=PROTOCOL_SCHEMA, head_bin_degrees=2.0, pixel_bin=64,
    maximum_group_members=64, q4_always_individually_flagged=False,
    old_Q4_post1_post2_always_individually_flagged=True,
    new_Q4_sampling_policy=group_margin.POLICY,
    representatives=['first','last','worst_margin','head_extrema','cut_extrema',
        'weakest_Q4_centerline','weakest_Q4_depth'],
...
class Pins:
    ...
    def add(self, spec):
        ...
        require(roots.resolve_evidence(p)==p,'Evidence outside explicit workspace/data roots')
...
def describe(row, pins):
    ...
    identity=dict(target=row['source_target'],family=row['source_family'],
        context_sha256=a['context']['sha256'],stage_initializer_anchor_entry=anchor['reference_entry_id'],
        stage_initializer_anchor_bank_sha256=anchor['reference_bank_sha256'],
        per_frame_body_and_root_source='authenticated_native_sample',body_joints=body,
    ...
    flags=[]
    if q4:
        if row.get('q4_group_review_authorized') is True:
            margins=group_margin.summarize(trace['conditional_background'])
            require(row['q4_review_margins']==margins,'Changed Q4 sampling margins')
            metrics.update(Q4_minimum_centerline_distance_px=margins['minimum_centerline_distance_px'],
                Q4_minimum_depth_gap_m=margins['minimum_depth_gap_m'])
            flags.extend(margins['flags'])
        else:flags.append('conditional_Q4_background_exception')
...
def select_groups(descriptors):
    ...
    for base in sorted(grouped):
        ...
        for offset in range(0,len(values),PROTOCOL['maximum_group_members']):
            ...
            for metric in ('Q4_minimum_centerline_distance_px','Q4_minimum_depth_gap_m'):
                eligible=[x for x in block if metric in x['metrics']]
                if eligible:select(min(eligible,key=lambda x:(x['metrics'][metric],x['sample_id'])),'weakest_'+metric)
            for x in block:
...
POST3_SHA = 'e3c48cee6948208c0ffaa11a5015e3b9f9e7e7b44e86f54159ec32c04649d982'
MARGIN_SHA = 'c815d6e11fc208a2de427f2662a4b5a539fdc9e54a6f1a36504c9141784149ea'


def verify_post3(result,pins):
    if result['schema'] != 'greenhouse.native848_diverse_bulk_q4_background_post.v3':return
    post=WORKSPACE/'examples/greenhouse_sim/sim_data/native848_diverse_bulk_q4_post_v3.py'
    require(result['implementation']==dict(path=str(post.resolve()),sha256=POST3_SHA)
        and result['state']=='fresh_original_bulk_background_exception_pending_group_or_individual_review'
        and result['all_candidates_require_individual_visual_review'] is False
        and result['authorized_group_or_individual_review_required'] is True
        and result['review_policy']==pin(POLICY_PATH),'Exact new post3 scope required')
    margin=Path(group_margin.__file__).resolve()
    require(result['group_margin_implementation']==dict(path=str(margin),sha256=MARGIN_SHA), 'Margin implementation differs')
    for spec in (result['implementation'],result['group_margin_implementation'],result['sampling_predeclaration']):
        require(result['source_bindings'].get(str(Path(spec['path']).resolve()))==spec['sha256'],'Unbound review source')
        pins.add(spec)
    policy=pins.json(result['root_policy_review'])
    require(policy['post_wrapper_sha256']==POST3_SHA and policy['group_margin_sha256']==MARGIN_SHA
        and policy['background_exception_policy_approved'] is True and policy['group_Q4_review_policy_approved'] is True
        and policy['blocking_findings']==[] and policy['training_approved'] is False
        and policy['selector_sha256']=='ec8ac3900457ee4358ad62da0ab4743e67f5731e8f1405bb073c813869efd7a8'
        and policy['sampling_predeclaration']==result['sampling_predeclaration'],'Exact reviewed group Q4 policy required')
    contract=result['effective_visual_review_contract']
    require(contract['schema']=='greenhouse.native848_Q4_group_review_contract.v1'
        and contract['user_policy']==pin(POLICY_PATH) and contract['sampling_policy']==group_margin.POLICY
        and contract['legacy_selector_individual_hint_superseded_for_new_post3_only'] is True
        and contract['all_flagged_require_individual_review'] is True
        and contract['any_bad_representative_holds_whole_group'] is True
        and contract['unsampled_individual_visual_review'] is False,'Effective visual review contract differs')


POST4_SHA = '2e124c0959e023f30eff3eb72b196f8c43d409cdb961146b29e233d4331c1e5c'

def verify_post4(result,pins):
    if result['schema'] != 'greenhouse.native848_diverse_bulk_q4_background_post.v4':return
    post=WORKSPACE/'examples/greenhouse_sim/sim_data/native848_diverse_bulk_q4_post_v4.py'
    require(result['implementation']==dict(path=str(post.resolve()),sha256=POST4_SHA)
        and result['state']=='fresh_original_bulk_background_exception_pending_group_or_individual_review'
        and result['all_candidates_require_individual_visual_review'] is False
        and result['authorized_group_or_individual_review_required'] is True
        and result['review_policy']==pin(POLICY_PATH),'Exact new post4 scope required')
    margin=Path(group_margin.__file__).resolve()
    require(result['group_margin_implementation']==dict(path=str(margin),sha256=MARGIN_SHA), 'Margin implementation differs')
    for spec in (result['implementation'],result['group_margin_implementation'],result['sampling_predeclaration']):
        require(result['source_bindings'].get(str(Path(spec['path']).resolve()))==spec['sha256'],'Unbound review source')
        pins.add(spec)
    policy=pins.json(result['root_policy_review'])
    require(policy['post_wrapper_sha256']==POST4_SHA and policy['group_margin_sha256']==MARGIN_SHA
        and policy['background_exception_policy_approved'] is True and policy['group_Q4_review_policy_approved'] is True
        and policy['blocking_findings']==[] and policy['training_approved'] is False
        and policy['selector_sha256']=='ec8ac3900457ee4358ad62da0ab4743e67f5731e8f1405bb073c813869efd7a8'
        and policy['sampling_predeclaration']==result['sampling_predeclaration'],'Exact reviewed group Q4 policy required')
    ...
    require(contract['schema']=='greenhouse.native848_Q4_group_review_contract.v1'
        and contract['user_policy']==pin(POLICY_PATH) and contract['sampling_policy']==group_margin.POLICY
        and contract['legacy_selector_individual_hint_superseded_for_new_group_post_only'] is True
        and contract['all_flagged_require_individual_review'] is True
        and contract['any_bad_representative_holds_whole_group'] is True
        and contract['unsampled_individual_visual_review'] is False,'Effective visual review contract differs')


POST5_SHA = '6cb097014dcfe757715bc9950d08f09823c3d1c28cddf54e76a0277b7f26190f'

def verify_post5(result,pins):
    if result['schema'] != 'greenhouse.native848_diverse_bulk_q4_background_post.v5':return
    post=WORKSPACE/'examples/greenhouse_sim/sim_data/native848_diverse_bulk_q4_post_v5.py'
    require(result['implementation']==dict(path=str(post.resolve()),sha256=POST5_SHA)
        and result['state']=='fresh_original_bulk_background_exception_pending_group_or_individual_review'
        and result['all_candidates_require_individual_visual_review'] is False
        and result['authorized_group_or_individual_review_required'] is True
        and result['review_policy']==pin(POLICY_PATH),'Exact new post5 scope required')
    margin=Path(group_margin.__file__).resolve()
    require(result['group_margin_implementation']==dict(path=str(margin),sha256=MARGIN_SHA), 'Margin implementation differs')
    for spec in (result['implementation'],result['group_margin_implementation'],result['sampling_predeclaration']):
        require(result['source_bindings'].get(str(Path(spec['path']).resolve()))==spec['sha256'],'Unbound review source')
        pins.add(spec)
    policy=pins.json(result['root_policy_review'])
    require(policy['post_wrapper_sha256']==POST5_SHA and policy['group_margin_sha256']==MARGIN_SHA
        and policy['background_exception_policy_approved'] is True and policy['group_Q4_review_policy_approved'] is True
        and policy['blocking_findings']==[] and policy['training_approved'] is False
        and policy['selector_sha256']=='ec8ac3900457ee4358ad62da0ab4743e67f5731e8f1405bb073c813869efd7a8'
        and policy['sampling_predeclaration']==result['sampling_predeclaration'],'Exact reviewed group Q4 policy required')
...
ADAPTER4_PINS = {
    'examples/greenhouse_sim/sim_data/native848_diverse_bulk_adapter_v4.py':'fd244adc37b6170ad5e29417e0d43ac1f18708c7d946f04e6d0fd68632cd9e4b',
    'examples/greenhouse_sim/sim_data/native848_multianchor_plan_v1.py':'5fc1e15bd493435fb2db50f11ab9a5c67c82d52daaae7caa3d7b31244f61e31c',
    'examples/greenhouse_sim/sim_data/native848_multianchor_sibling_worker_v1.py':'160089f6fb87628ca7d56962cb2552eee3fc4be22eaa75c9569190aa88311485',
    'examples/greenhouse_sim/sim_data/native848_multianchor_parallel_predecessor_v1.py':'d906cbedbd909be5d1981992d1db326680998e17c8fb1493a0c57504ae13bc9e',
    'examples/greenhouse_sim/sim_data/native848_bulk_sibling_gate_v5.py':'3766afb02212dfa0bb432bb61c6dc9b3e45d7fbd259d778be6e7bc2b9a7150d4',
    'data/sim_data/diagnostics/collection_20k_848_20260916_v1/run_native848_multianchor_parallel_v1.py':'526f6cd95bc405b72c4e2f14af48df989d57e029f7e2efd87518ccd562df2566',
}


def verify_adapter4(result,pins):
    if result['schema'] != 'greenhouse.native848_diverse_bulk_q3_adapter.v4':return
    path=WORKSPACE/'examples/greenhouse_sim/sim_data/native848_diverse_bulk_adapter_v4.py'
    require(result['implementation']==dict(path=str(path),sha256=ADAPTER4_PINS['examples/greenhouse_sim/sim_data/native848_diverse_bulk_adapter_v4.py'])
        and result['native_source_route']=='completed_parallel_multianchor_slot'
        and result['state']=='original_bulk_q3_normalized_pending_group_or_individual_review'
        and result['authorized_group_or_individual_review_required'] is True,'Exact parallel adapter4 scope differs')
    for relative,h in ADAPTER4_PINS.items():
        p=(WORKSPACE/relative).resolve();require(result['source_bindings'].get(str(p))==h,'Unbound parallel adapter4 source chain')
        pins.add(dict(path=str(p),sha256=h))
    for key in ('parallel_parent_result','parallel_parent_complete','parallel_parent_intent','parallel_slot_ticket','parallel_slot_release'):
        spec=result[key];require(result['source_bindings'].get(str(Path(spec['path']).resolve()))==spec['sha256'],'Unbound parallel completion receipt')
        pins.add(spec)


ADAPTER5_PINS = {
    'examples/greenhouse_sim/sim_data/native848_data_roots_v1.py':'b3b5d6c77385b2776a6997dd7dbe14f1aae8e1d093a45140d0f2c033fc2c8d83',
    'examples/greenhouse_sim/sim_data/native848_storage_environment_v1.py':'826f3e3c85e886d9c6b4e21fbdf8164d9eb3e1b50c395fb91c3ef90ed456667b',
    'data/sim_data/diagnostics/collection_20k_848_20260916_v1/run_bulk_cpu_v6.py':'54d077837ece88c6612bdef5b2b10076c167b7939daa73e6b251f8be4df8e955',
    'examples/greenhouse_sim/sim_data/native848_diverse_bulk_adapter_v5.py':'04b997f71abfcd75402c1a64bc1f9570c75dc0a0f761e6c32b11e554cdad6759',
    'examples/greenhouse_sim/sim_data/native848_multianchor_plan_v1.py':'5fc1e15bd493435fb2db50f11ab9a5c67c82d52daaae7caa3d7b31244f61e31c',
    'examples/greenhouse_sim/sim_data/native848_multianchor_sibling_worker_v2.py':'5abb477b94f8d32ff1f52969e38fd1a102b1d9956bb0c245147f11d517abe793',
    'examples/greenhouse_sim/sim_data/native848_multianchor_parallel_predecessor_v2.py':'b2c9e49a2afb8a951064ad1bbbe66ed4039f14f34e5e27b5c2dcebf0aebe17ee',
    'examples/greenhouse_sim/sim_data/native848_bulk_sibling_gate_v5.py':'3766afb02212dfa0bb432bb61c6dc9b3e45d7fbd259d778be6e7bc2b9a7150d4',
    'data/sim_data/diagnostics/collection_20k_848_20260916_v1/run_native848_multianchor_parallel_v2.py':'bd0e3d54ef08f7189366f845962e7deda1ff45e29aa6de3f17dbf7dbe06972fa',
}


def verify_adapter5(result,pins):
    if result['schema'] != 'greenhouse.native848_diverse_bulk_q3_adapter.v5':return
    path=WORKSPACE/'examples/greenhouse_sim/sim_data/native848_diverse_bulk_adapter_v5.py'
    require(result['implementation']==dict(path=str(path),sha256=ADAPTER5_PINS['examples/greenhouse_sim/sim_data/native848_diverse_bulk_adapter_v5.py'])
        and result['native_source_route']=='completed_parallel_multianchor_slot'
        and result['state']=='original_bulk_q3_normalized_pending_group_or_individual_review'
        and result['authorized_group_or_individual_review_required'] is True,'Exact parallel adapter5 scope differs')
    for path,h in roots.bindings().items():
        require(result['source_bindings'].get(path)==h,'Unbound explicit data-root contract')
        pins.add(dict(path=path,sha256=h))
    for relative,h in ADAPTER5_PINS.items():
        p=(WORKSPACE/relative).resolve();require(result['source_bindings'].get(str(p))==h,'Unbound parallel adapter5 source chain')
        pins.add(dict(path=str(p),sha256=h))
    for key in ('parallel_parent_result','parallel_parent_complete','parallel_parent_intent','parallel_slot_ticket','parallel_slot_release'):
        spec=result[key];require(result['source_bindings'].get(str(Path(spec['path']).resolve()))==spec['sha256'],'Unbound parallel completion receipt')
        pins.add(spec)
...
def inventory(result_path,result_sha256,hold_snapshot,policy_spec,pins):
    ...
    require(result['schema'] in ('greenhouse.native848_diverse_bulk_q3_adapter.v1',
        ...
        'greenhouse.native848_diverse_bulk_q4_background_post.v2',
        'greenhouse.native848_diverse_bulk_q4_background_post.v3',
        'greenhouse.native848_diverse_bulk_q4_background_post.v4',
        'greenhouse.native848_diverse_bulk_q4_background_post.v5',
        'greenhouse.native848_diverse_bulk_q3_adapter.v5',
        'greenhouse.native848_diverse_bulk_q3_adapter.v4',
    ...
    verify_post3(result,pins)
    verify_post4(result,pins)
    verify_adapter4(result,pins)
    verify_post5(result,pins)
    verify_adapter5(result,pins)
    require(digest(roots.__file__)=='b3b5d6c77385b2776a6997dd7dbe14f1aae8e1d093a45140d0f2c033fc2c8d83','Data-root implementation changed')
    for path,h in roots.bindings().items():pins.add(dict(path=path,sha256=h))
    require(policy_spec==pin(POLICY_PATH),'Exact saved user policy required')
    policy=pins.json(policy_spec)
    if result['schema'] in ('greenhouse.native848_diverse_bulk_q3_adapter.v2','greenhouse.native848_diverse_bulk_q3_adapter.v3','greenhouse.native848_diverse_bulk_q3_adapter.v4','greenhouse.native848_diverse_bulk_q3_adapter.v5'):
    ...
    for row in rows:
        require((row.get('q4_group_review_authorized') is True)==(result['schema'] in ('greenhouse.native848_diverse_bulk_q4_background_post.v3','greenhouse.native848_diverse_bulk_q4_background_post.v4','greenhouse.native848_diverse_bulk_q4_background_post.v5')),
            'New Q4 group authorization cannot be applied to an old result')
        if aliases(row)&held:
...
def prepare_group_review(normalized_result_path,result_sha256,hold_ledger_path,output):
    ...
    require(roots.resolve_data(out,roots.REVIEW_ROOTS)==out,'Output outside explicit review roots')
    ...
    write(out/'visual_packet.json',dict(schema='greenhouse.native848_diverse_group_visual_packet.v5',
