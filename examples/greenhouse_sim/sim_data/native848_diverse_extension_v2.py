...
SCHEMA='greenhouse.native848_diverse_extension.v2'
REQUEST_SCHEMA='greenhouse.native848_diverse_extension_request.v2'
...
def inspect(request_path,request_sha256):
    ...
    if previous is not None:
        ...
        require(prior['schema']==SCHEMA and prior['state']=='diverse_cumulative_batch_with_explicit_review_scopes_complete'
            and prior.get('CPU_fixture_only') is not True and prior['base_result']==request['base_result']
            and prior['source_bindings'].get(str(Path(__file__).resolve()))==digest(__file__),
            'Prior cumulative result differs from this frozen writer/starting snapshot')
    ...
    for spec in request['admissions']:
        ...
        if is_q4:
            ...
            route='bulk_q3'
            version2=result['schema']=='greenhouse.native848_diverse_bulk_q3_adapter.v2'
            expected_impl='native848_diverse_bulk_adapter_v2.py' if version2 else 'native848_diverse_bulk_adapter_v1.py'
            expected_state='original_bulk_q3_normalized_pending_group_or_individual_review' if version2 else 'original_bulk_q3_normalized_pending_individual_review'
            require(result['schema'] in ('greenhouse.native848_diverse_bulk_q3_adapter.v1','greenhouse.native848_diverse_bulk_q3_adapter.v2')
        ...
        if result['schema']=='greenhouse.native848_diverse_bulk_q3_adapter.v2':
            require(result['implementation']['sha256']==FROZEN_Q3_ADAPTER_V2_SHA
                and result['review_policy']==request['group_user_policy']
                and result['authorized_group_or_individual_review_required'] is True,
                'Explicit reviewed adapter-v2 source and user policy required')
        ...
        if group_mode:
            ...
            require(request['implementation_bindings'].get(str(helper))==digest(helper)==FROZEN_GROUP_HELPER_SHA,
