...
def owned_allowlist(coordinator, slots):
    """Slots are immutable command+native receipts from no more than three launches."""
    require(len(slots)<=3 and len({s['slot'] for s in slots})==len(slots)
        and all(s['slot'] in (0,1,2) for s in slots),'Exactly bounded unique slots required')
