...
SCHEMA = 'greenhouse.native848_fast_profile_benchmark_plan.v1'
SAMPLE = 'greenhouse.native848_fast_profile_benchmark_sample.v1'
...
def recording_writer(rep,unrequested_folder):
    ...
    class DiagnosticWriter(rep.Writer):
        def __init__(self):
            self.version='native848-fast-profile-benchmark-v1'
...
def full_scene(app, output, plan, scene_name, loaded):
    ...
    try:
        for item in plan['schedules'][scene_name]:
            ...
            print('FAST848_PROFILE_V1',scene_name,item['name'],receipt.get('capture_valid',False),receipt['passed_actual_automated_criteria'],flush=True)
    finally:
...
def audit(output,plan):
    ...
    actual_files={str(p.resolve()) for p in output.rglob('*') if p.is_file() and p.name not in ('result.json','evidence_bindings.json','audit_fast_profile_v1.json')}
...
def cpu_tests():
    ...
    import tempfile
    from .native_instances import normalize_payload
    with tempfile.TemporaryDirectory(prefix='fast848_profile_v1_cpu_',dir=D) as temporary:
...
def main(argv=None):
    ...
    if args.action=='audit':
        ...
        require(not (output/'audit_fast_profile_v1.json').exists(),'Create-only audit required')
        write_json(output/'audit_fast_profile_v1.json',report);return
