"""Normalize balanced serial original bulk Q3 batches; unchanged per-image checks."""
...
SCHEMA='greenhouse.native848_diverse_bulk_q3_adapter.v3'
...
OWNER_SHA='c92ec4cd9409dfbaed657bd5344ea9cd78380deb1c7cc77c62c135ebea1237b8'
...
def load_owned(trial,cpu,native_result_sha256,cpu_result_sha256):
    ...
    owner=artifact(H/'run_native848_multianchor_v1.py',pins,OWNER_SHA)
    ...
    require(argument(command,'-m')=='sim_data.native848_multianchor_worker_v1'
    ...
    require(plan['generated_geometry_used'] is False and plan['per_target_view_cap'] is None
        ...
        and 1<=len(plan['schedule'])<=512,'Bounded original production plan required')
...
def run(trial,cpu,*,native_result_sha256,cpu_result_sha256,output):
    ...
    result=dict(schema=SCHEMA,state=STATE,input_route='bulk_q3',created_utc=datetime.now(timezone.utc).isoformat(),
        ...
        review_policy=user_policy,source_cap_reset=False,
