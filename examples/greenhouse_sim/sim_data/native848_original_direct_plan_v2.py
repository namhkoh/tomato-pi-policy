...
from . import native848_original_reference_v2 as pair
from .native_view_pose import bounded_reference_root

SCHEMA = 'greenhouse.original848_reference_direct_camera_batch_plan.v2'
SAMPLE_SCHEMA = 'greenhouse.original848_reference_direct_camera_sample.v2'
RESULT_STATE = 'original848_reference_direct_batch_captured_pending_independent_audit'
PROFILE = 'original848_reference_direct_cached_poses_established56_every_frame.v2'


def implementation_bindings():
    root = Path(__file__).resolve().parent
    roots = (root.parent, root.parents[1])
    pending = [root/name for name in ('native848_original_direct_plan_v2.py',
        'native848_original_direct_worker_v2.py', 'native848_original_direct_audit_v2.py',
        'native848_original_direct_admission_v2.py')]
    found = {}
    while pending:
...
def check(plan, *, full=False):
    ...
    require(cache['schema'] == 'greenhouse.original848_cached_embodied_camera_poses.v2'
        ...
        and cache['source_cap_reset'] is False and cache['native_launched'] is False,
        'Original-only cache required')
    verify_bindings(cache['source_bindings'])
    anchor_path = cache['anchor_reference_path']
    require(sha256(anchor_path) == cache['anchor_reference_sha256'], 'Changed reconstructed-scene anchor')
    ...
    for name in ids:
        ...
        require(target in source_rows and rec['source_row'] == source_rows[target]
            and rec['source_target'] == target and rec['anchor_reference_path'] == anchor_path
            and rec['anchor_reference_sha256'] == cache['anchor_reference_sha256'],
