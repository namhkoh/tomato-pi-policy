"""Diagnostic two-frame far-component merge replay; no training admission.
...
def apply_profile(rep,settings,profile):
    """Observation-only original renderer settings under truthful production6 schema."""
    api.check_profile(profile)
    observed={key:jsonable(settings.get(key)) for key in profile['render_settings']}
    require(observed==profile['render_settings'],'Qualified unchanged original renderer settings differ')
    return observed


def population_metadata(split,*,native):
    return api.metadata(split,native=native)
...
def require_native_census(census,cpu):
    ...
    require(saved==fingerprint(identity),'Native merged render census fingerprint differs')
    logical=api.read(cpu['logical_original_census'])
    require(census['schema']=='greenhouse.native848_farmerge_native_render_census.v1'
        and census['logical_original_census_sha256']==logical['deterministic_census_sha256']
        and census['logical_plant_slots']==144 and census['logical_anatomy_components']==61386
        and census['actual_native_render_meshes']==11177 and census['actual_native_retained_original_meshes']==11034
        and census['actual_native_coarse_groups']==143 and census['ordinary_annotation_compatible'] is False
        and census['actual_native_packed_geometry_normals_UVs_materials_match_CPU_artifacts'] is True
        and census['actual_native_retained_original_world_matrices_exact'] is True,
        'Actual merged render census must be separate from full logical anatomy')


def capture(app, output, plan_path, plan, checked):
    import omni.usd
    import omni.replicator.core as rep
    from .native848_farmerge_pair_scene_v1 import prepare_native_scene, verify_native_scene, OriginalCollisionBridge, classify_mapping
    ...
    scene = prepare_native_scene(app, anchor, checked)
    ...
    catalogue = scene['logical_catalogue']
    require(len(catalogue) == scene['logical_counts']['components'], 'Complete logical component population changed')
    ...
    geometry = OriginalCollisionBridge(scene)
    monitor = sink = None
    ...
    try:
        ...
        require(verify_native_scene(scene)==scene['scene_policy_evidence'],'Merged native geometry changed during warmup')
        ...
        context = dict(**population_metadata(checked['capture_split'],native=True), accepted_training_increment=0, schema=api.CONTEXT_SCHEMA, plan_path=str(plan_path.resolve()), plan_sha256=sha256(plan_path),
            ...
            source_pose_profile_evidence=plan['source_pose_profile_evidence'],source_geometry_plan=plan['source_geometry_plan'],
            source_geometry_preflight=plan['source_geometry_preflight'],production_render_budget_changed_only=False,
            original_collision_CPU_evidence_reused=True,fresh_original_collision_stage_checked_each_native_pose=True,
            merged_render_prototype=plan['prototype'],merged_render_CPU_closure=plan['prototype_closure'],
            logical_original_census_path=str(output/'logical_original_census.json'),logical_original_census_sha256=sha256(output/'logical_original_census.json'),
            paired_reference_selection=plan['paired_reference_selection'],
        ...
        for capture_index, (item, rec) in enumerate(zip(plan['schedule'],records)):
            ...
            try:
                ...
                screen = geometry.check(rec,pose,cal)
                ...
                proof = dict(schema='greenhouse.native848_farmerge_original_collision_geometry_proof.v1', pose_key=key,
                ...
                value = dict(renderer_id_to_prim={str(k):v for k,v in mapping.items()},
                    scope='all_observed_renderer_IDs_with_explicit_near_component_or_far_coarse_group_ownership',
                    plant_ID_ownership=classify_mapping(scene,mapping),individual_far_component_pixels_claimed=False)
                ...
                observation = dict(**population_metadata(checked['capture_split'],native=True), accepted_training_increment=0, schema=api.OBSERVATION_SCHEMA,observation_id=name,capture_index=capture_index,
                    ...
                    source_pose_split=checked['capture_split'],capture_role=api.ROLE,profile=profile['profile'],
        ...
        require(verify_native_scene(scene)==scene['scene_policy_evidence'],'Merged native geometry changed during capture')
        ...
        manifest = dict(**population_metadata(checked['capture_split'],native=True),schema=api.MANIFEST_SCHEMA,context_path=str(context_path.resolve()),
            ...
            stage_count=2,render_stage_count=1,original_collision_stage_count=1,render_product_count=1,automatic_retries=False,training_approved=False,
            accepted_training_increment=0)
        require_scope(manifest,native=True)
        save_json(output/'batch_manifest.json',manifest)
        return dict(**population_metadata(checked['capture_split'],native=True),schema=api.CAPTURE_SCHEMA,state=api.RESULT_STATE,manifest_path=str((output/'batch_manifest.json').resolve()),
...
def optout_bindings():
    ...
    bindings[str(Path(api.__file__).resolve())]='ed6889131dece09a56298d4c4971b3a9b88913cae2099f099b42fd8b7b795262'
...
def native_command(args):
    return ['D:/isaac-sim-6.0.1/python.bat','-B','-u','-m','sim_data.native848_farmerge_pair_worker_v1',
...
def record_native_identity(args):
    ...
    return dict(schema='greenhouse.native848_farmerge_pair_worker_identity.v1',owner_identity=owner,
...
def validate_native_identity(value,command,*,owner_identity=None):
    require(value['schema']=='greenhouse.native848_farmerge_pair_worker_identity.v1'
        and value['same_snapshot_identity_checks'] is True and value['command']==command
        and command[1:5]==['-B','-u','-m','sim_data.native848_farmerge_pair_worker_v1'],
...
def runtime_evidence(phase,config,native_pin):
    ...
    return dict(schema='greenhouse.native848_farmerge_pair_runtime.v1',phase=phase,
...
def validate_optout_capture(capture,value,context):
    ...
    for phase in ('start','end'):
        ...
        require(evidence['schema']=='greenhouse.native848_farmerge_pair_runtime.v1'
...
def main(argv=None):
    ...
    try:
        ...
        profile=read_json(plan['profile_evidence']['path']);api.check_profile(profile)
        if profile['schema']==api.PROFILE_SCHEMA:
        ...
        api.validate_cpu(cpu,plan,args.plan,args.plan_sha256)
        require(cpu['schema']==api.CPU_SCHEMA
        ...
        save_json(output/'failure.json',dict(state='native848_farmerge_diagnostic_failed',error=traceback.format_exc(),
