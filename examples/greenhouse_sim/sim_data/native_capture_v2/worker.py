"""ISOLATED WIDER-VIEW EXPERIMENT; no production approval or navigation.

Keeps the same guarded native capture, using an explicitly bounded outward
robot-base snapshot policy. Actual floor/head-joint/geometry/native-ID/Z and
visibility checks are unchanged. Uses the established reference/8-frame options.
...
import traceback
import numpy as np
from sim_data.dataset_review import read_json,write_json,require,verify_bindings
from sim_data.depth_preview import sha256
from sim_data.native_view_plan import check
from sim_data.capture_sensor import HIRES_RESOLUTION,calibration_for_native_resolution,sensor_profile
from sim_data.capture_contract import jsonable,project,depth_evidence,write_sample
from sim_data.native_sensor_payload import validate_native_static,decode_native_instances
from sim_data.native_budget import REFERENCE,TRIAL,evidence as budget_evidence


def collect(app,output,plan,base,*,profile_render=False,instance_backend='legacy',render_budget=REFERENCE):
    import omni.replicator.core as rep
    import omni.timeline
    from sim_data.native_scene import prepare_native_scene
    from sim_data.generated_capture import substitute_plant
    from sim_data.capture_scene import calibration,target_world_geometry,mounted_camera_to_head
    from sim_data.native_capture_v2.pose import set_reference_snapshot
    from sim_data.capture_pilot import make_writer,step_payload,source_hashes
    from sim_data.capture_viewpoints import component_catalogue
    from sim_data.capture_visibility import component_masks,interval_visibility,view_quality,write_visibility
    from sim_data.static_geometry_cache import StaticGeometryScreenCache
    from sim_data.static_guard import StaticSceneMonitor
    from sim_data.review_camera import HEAD_CAMERA
    from sim_data.native_clear_labels import derive
    from sim_data.automated_native_review import trace_review
    from sim_data.native_greenhouse_pair import assert_same_camera
    from sim_data.native_multitarget_plan import capture_jobs
    ...
    if instance_backend=='legacy':writer=make_writer(rep,include_instances=True,instance_backend='legacy')
        ...
        from sim_data.native_instance_adapter import make_native_writer
    ...
    try:
        for base,target_plan,spec in capture_jobs(plan,anchor):
            ...
            if profile_render:
                from sim_data.native_render_probe import noise_probe
...
def main():
    ...
    if a.batch_plan:
        from sim_data.native_capture_v2.plan import check as check_batch
        base=check_batch(plan,replay_geometry=False)
    else:base=check(plan)
    require(a.batch_plan is not None,'Wider experiment requires its explicit batch plan')
    ...
    write_json(output/'request.json',dict(created_utc=datetime.now(timezone.utc).isoformat(),
        ...
        experiment_script_sha256=experiment_script_sha256,pose_policy=plan['pose_policy'],
    ...
    try:
        ...
        require(sha256(Path(__file__))==experiment_script_sha256,'Experiment worker changed')
        result.update(experiment_script_sha256=experiment_script_sha256,pose_policy=plan['pose_policy'])
