...
def audit_capture(capture, *, plan_path, plan_sha256, result_sha256):
    ...
    for probe in result['render_probes']:
        name = probe['sample_id']
        require(name in plan['long_reference_probe_ids'] and probe['saved_training_candidate'] == 'long_reference'
            and probe['comparison']['geometry_and_identity_equal'] is True
            and probe['short_callback_sequence'] < probe['reference_callback_sequence'], 'Invalid reference comparison')
        pp = capture/'render_probes'/(name+'.json')
        require(read_json(pp) == probe, 'Render probe changed')
        pins[str(pp)] = sha256(pp)
        for suffix, field in (('_8.png', 'short_rgb_sha256'), ('_reference.png', 'reference_rgb_sha256')):
            p = capture/'render_probes'/(name+suffix)
            require(sha256(p) == probe[field] and image_array(p).shape == (408, 848, 3), 'Changed native render probe')
            pins[str(p)] = sha256(p)
    qualified = len(result['render_probes']) == 3 and all(p['comparison']['passed'] for p in result['render_probes'])
    require(result['short_profile_comparisons_passed'] is qualified, 'Changed render comparison result')
    probes = {p['sample_id']: p for p in result['render_probes']}
    previous = None
    records = []
    for index, name in enumerate(expected_ids):
        ...
        budget = (56 if index == 0 else 8) + (56 if name in probes else 0)
        ...
        previous = fresh
        if name in probes:
            require(fresh['callback_sequence'] == probes[name]['reference_callback_sequence']
                and sha256(folder/'inputs/rgb.png') == probes[name]['reference_rgb_sha256'], 'Sample is not reference callback')
        ...
        records.append(dict(sample_id=name, target_id=row['target_id'], source_target=pair['conservative_view_cap_group'],
            ...
            metric_cut_geometry_recomputed=True, render_profile_qualified=budget >= 56 or qualified,
            accepted_training_increment=0))
    verify_bindings(pins)
    return dict(schema='greenhouse.native848_direct_buffer_geometry_audit.v1',
        ...
        native_frames=len(records), pose_search_attempts=0, training_approved=False,
