...
SCHEMA = 'greenhouse.native848_fast_profile_benchmark_plan.v2'
SAMPLE = 'greenhouse.native848_fast_profile_benchmark_sample.v2'
...
PROFILE = 'rt2_fxaa_renderclock_1over60_reset_each_request.v1'
...
def single_request(rep, writer, subframes):
    ...
    try:
        ...
        writer.last_step_timing=dict(orchestrator_step_wall_seconds=time.perf_counter()-started,
            app_update_event_count=len(updates),timeline_before=clock_before,
            timeline_after=timeline.get_current_time(),delta_time_seconds=CLOCK_DELTA,
...
def verify_clock_request(evidence, folder):
    ...
    for i,request in enumerate(evidence['requests']):
        ...
        require(reference==request['reference_time'],'Clock reference differs from saved payload')
        previous=request['previous_reference_time']
        if previous is not None:
            require(abs(float(Fraction(*reference)-Fraction(*previous))-CLOCK_DELTA)<1e-7,
                'Native timestamp did not advance by the exact requested render clock')
...
def prepare():
    plan = dict(schema=SCHEMA, resolution=[848, 408], budgets=BUDGETS,
        ...
        installed_source_bindings={str(INSTALLED/name):sha256(INSTALLED/name) for name in ('settings.py','orchestrator.py')}, sources={k:dict(path=str(p.resolve()), sha256=sha256(p)) for k,p in SOURCES.items()},
...
def check(plan, full=False):
    ...
    require(plan['installed_source_bindings']=={str(INSTALLED/name):sha256(INSTALLED/name) for name in ('settings.py','orchestrator.py')},'Changed installed renderer/orchestrator API')
...
def recording_writer(rep,unrequested_folder):
    ...
    class DiagnosticWriter(rep.Writer):
        def __init__(self):
            self.version='native848-fast-profile-benchmark-v2'
...
def freshness(payload, cal, before, after, sequence, previous=None, expect_change=False):
    ...
    if previous is not None:
        ...
        from fractions import Fraction
        require(Fraction(*reference)>Fraction(*previous['reference_time']),'Stale native reference timestamp')
        if expect_change:
...
def full_scene(app, output, plan, scene_name, loaded):
    ...
    try:
        for item in plan['schedules'][scene_name]:
            ...
            print('FAST848_PROFILE_V2',scene_name,item['name'],receipt.get('capture_valid',False),receipt['passed_actual_automated_criteria'],flush=True)
    finally:
...
def audit(output,plan):
    ...
    actual_files={str(p.resolve()) for p in output.rglob('*') if p.is_file() and p.name not in ('result.json','evidence_bindings.json','audit_fast_profile_v2.json')}
...
def cpu_tests():
    ...
    import tempfile
    from .native_instances import normalize_payload
    with tempfile.TemporaryDirectory(prefix='fast848_profile_v2_cpu_',dir=D) as temporary:
        ...
        evidence=dict(requested_steps=[BUDGETS[0]],settings=PROFILE_SETTINGS,hidden_settling=False,callbacks=[event],
            ...
            requests=[dict(subframes=BUDGETS[0],native_requests=1,request_index_before=0,request_index_after=1,
                ...
                previous_reference_time=None,reference_time=[0,1],step_timing=dict(delta_time_seconds=CLOCK_DELTA,
                    wait_for_render=True,app_update_event_count=1))])
...
def main(argv=None):
    ...
    if args.action=='audit':
        ...
        require(not (output/'audit_fast_profile_v2.json').exists(),'Create-only audit required')
        write_json(output/'audit_fast_profile_v2.json',report);return
