...
from . import native848_query_selection_v2 as queries
...
class Admission:
    ...
    def frame(self, observation_path):
        ...
        label, trace, selection = queries.annotate_v2(meta,target['anatomy_report'],rgb,depth,valid,
        ...
        decision = ('exclude_duplicate_or_prior_hold' if duplicate else 'exclude_local_clarity' if not local
            else 'hold_query_trace_or_foreground_clearance' if not traced else 'hold_workspace' if not reachable
    ...
    def finish(self):
        ...
        result = dict(schema='greenhouse.original848_bulk_admission.v3', annotation_epoch=queries.ANNOTATION_EPOCH,
