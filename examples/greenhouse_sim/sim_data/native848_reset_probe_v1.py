"""Opt-in two-pose accumulation-reset diagnostic. Never admits TRAIN samples."""
...
SCHEMA = 'greenhouse.native848_reset_probe_plan.v1'
SAMPLE = 'greenhouse.native848_reset_probe_sample.v1'
STATE = 'native848_reset_probe_complete_unqualified_diagnostic_only'
CASES = ['original_reviewed_clear', 'generated_known_fruit_occlusion']
BRANCHES = {'reset56_reference': [8]*7, 'reset56_repeat': [8]*7,
            'reset8': [8], 'reset16': [16]}
SETTINGS = ['/rtx/rendermode', '/rtx/post/aa/op', '/rtx/post/dlss/execMode',
...
def prepare(direct_path, native_evidence, original_pair_path, clear_admission_path, visual_review_path):
    ...
    plan = dict(schema=SCHEMA, direct_plan_path=str(path), direct_plan_sha256=sha256(path),
        ...
        maximum_saved_observations=8, short_branches_require_stable_long_reference=True,
...
def check(plan, full=False):
    require(plan['schema'] == SCHEMA and plan['cases'] == CASES and plan['branches'] == BRANCHES
        ...
        and plan['maximum_poses'] == 2 and plan['maximum_saved_observations'] == 8
                ...
                for branch, steps in BRANCHES.items():
                    ...
                    meta = dict(schema_version=SAMPLE,sample_id=sample_id,case_id=case_id,branch=branch,
                        ...
                        synchronization=dict(method='frozen_scene_single_native_writer_payload',
                            ...
                            actual_native_requests=writer.request_index-requests,render_seconds=render_seconds),
...
def audit(output, plan):
    ...
    for case_result in result['cases']:
        ...
        require(names == list(BRANCHES)[:len(names)] and len(names) in (2,4), 'Invalid branch execution order')
        for name in names:
        ...
        require(actual==case_result['comparisons'] and (len(names)==4)==actual['reference_repeat']['passed'],
