...
from . import native848_owned_retirement_v2 as retirement
...
def main(argv=None):
    ...
    require(ticket['schema']=='greenhouse.native848_pilot_parallel_ticket.v3' and type(ticket['worker_count']) is int and ticket['worker_count'] in (2,3,4)
    ...
    try:
        ...
        def check_stop():
            require(not (control/'CANCEL_BEFORE_SIMULATIONAPP').exists(),'Coordinator cancelled before SimulationApp')
        admission=retirement.inventory(control.parent,coordinator,release['slots'],protected_slot=ticket['slot'],check_stop=check_stop)
