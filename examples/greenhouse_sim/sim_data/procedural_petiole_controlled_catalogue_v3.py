...
from .procedural_petiole_controlled_v3 import VERSION, plan_change_explicit, verify_components, code_bindings
from .procedural_proximal_shear_v1 import proximal_mesh_identity
...
def load_for_inspection(directory, source_plan_path):
    ...
    proximal=proximal_mesh_identity(safe_asset(source_path.parent,source['report']['components'][key]['file']),
        safe_asset(directory,report['components'][key]['file']),source['report']['components'][key],report['components'][key],change['warp'])
    require(proximal==receipt['proximal_mesh_identity'],'Proximal mesh identity replay differs')
    ...
    return dict(schema_version='greenhouse.proximal_preserved_control_inspection_catalogue.v3', directory=str(directory),
        ...
        explicit_control=receipt['explicit_control'], protected_attribute_proof=proof,proximal_mesh_identity=proximal)
