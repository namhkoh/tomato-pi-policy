"""Read-only inventory for post-reboot serial-phase original_batch.v1 receipts.
...
from . import serial_phases_reboot_v2 as q
from ..native_original_capture import audit, contracts as oc

POLICY = 'greenhouse.original_reboot_phase_observed_adapter.v2'
PRODUCER_SHA256 = '81271566c5d139ec57ccc0437e3c2c95f1756719615bd6d91f01a129e5d3bcfe'
...
def _saved_predecessor(b, root, request, state):
    """Authenticate saved reboot handoff without demanding the old boot is live."""
    pred = request['predecessor']
    config, roots, bindings = q.reboot.validate_predecessor(pred)
    b.mapping(bindings)
    prior = _bound(b, root / 'predecessor_completion.json')
    _flags(prior)
    expected = q.reboot.saved_completion(pred, config)
    oc.require(prior == expected, 'Saved reboot handoff differs')
    b.mapping(prior['bindings'])
    b.protected_roots.update(str(inv._path(p)) for p in roots)
    return prior
