"""Explicit query-V2 annotation epoch over frozen compact_views sensing flow.

One native greenhouse stage/product, unchanged actual robot poses, ID/optical-Z,
render budgets, source checks and lossless storage. All rendered frames persist;
no strict-only persistence or diversity/source-cap reset is implemented here.
Full deterministic selection evidence is nested in label.json; the unchanged
composite trace occupies query_trace.json. No third sidecar or global rebinding.

Initial CLI accepts --batch-plan only (including an existing one-target batch).
Caller pins --plan-sha256, --annotation-policy-sha256 and the existing completed
--storage-qualification/--storage-qualification-sha256. Defaults remain legacy
instances and reference56; existing fast/trial choices are explicit experiments.
Post-exit replay: sim_data.native_dataset.query_audit_v2. A later compatible
queue/inventory adapter must bind this new epoch; this worker grants no approval.
"""
...
from .query_audit_v2 import (
    annotate_for_storage, annotation_fields, annotation_policy,
    implementation_bindings as query_implementation_bindings,
    verify_epoch_documents, CAPTURE_STATE, SAMPLE_SCHEMA, WORKER_MODULE)
...
def implementation_bindings():
    ...
    return query_implementation_bindings()
