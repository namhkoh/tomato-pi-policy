"""Opt-in compact storage copy of frozen native_generated_views (573054884fa4).

Only serialization and explicit storage qualification/bindings differ. Standard
native_view_pose, renderer, physics/subframes, native identity/Z, scene/geometry,
labels and trace gates are unchanged. No V2 wider pose or renderer-probe mode.
The caller must pin a completed same_callback_qualification.json.
...
def implementation_bindings():
    """Pin this collector plus the explicit qualified storage implementation."""
    require(sha256(Path(__file__)) == _LOADED_SELF_SHA256, 'Compact collector changed')
    return dict(compact_qualification.implementation_bindings(),
                **{str(Path(__file__).resolve()): _LOADED_SELF_SHA256})
