"""Explicit selective-persistence V3; native/query V2 flow remains frozen.

Default --persistence all_frames. --persistence strict_candidates_only retains
strict candidates and pinned 1/64 donor/hold-or-exclude hashed controls. Every
callback still runs native evidence, geometry, annotation and source checks.
Validated callbacks, NOT retained samples, advance warmup and freshness.

--qualification-witness-save-all stores the same callback under the fixed
qualification_witness/ directory (max 12 planned views), without extra renders.
Primary/witness bindings are separate. Missing pixels have only producer
receipts, never fake sample hashes or independently replayed annotation claims.
Metadata/labels keep the V2 annotation epoch; persistence is request/result-only.

Requires the V2 CLI pins plus --persistence-policy-sha256. Not native qualified,
not registered with a queue/inventory, and no boundary_bindings integration.
"""
...
from ..capture_contract import jsonable,project,depth_evidence
...
from .query_audit_v2 import annotate_for_storage, annotation_fields, annotation_policy, SAMPLE_SCHEMA
from . import query_audit_v3 as audit3
from .query_audit_v3 import (ALL_FRAMES, STRICT_ONLY, CAPTURE_STATE, CALLBACK_STATE,
                             WORKER_MODULE, verify_epoch_documents)

STORAGE_BACKEND = compact_qualification.STORAGE_BACKEND
_LOADED_SELF_SHA256 = sha256(Path(__file__))


def implementation_bindings():
    """Pin this worker, auditor, selector/policy and qualified frozen storage."""
    require(sha256(Path(__file__)) == _LOADED_SELF_SHA256, 'Compact collector changed')
    return audit3.implementation_bindings()


def persist_callback(output, name, rgb, depth, valid, metadata, instances, mapping, catalogue,
                     components, organs, target_mask, label, trace, *, plan_sha256,
                     persistence=ALL_FRAMES, qualification_witness=False):
    """Persist only after full native annotation; never invent missing files.

    Witness mode calls the SAME frozen writer on the SAME callback. It creates
    no additional sensor/product/render request and never changes metadata.
    """
    audit3.configuration(persistence, qualification_witness)
    audit3.candidate_name(name)
    audit3.validate_live_buffers(metadata, rgb, depth, valid, instances, components, organs, target_mask, catalogue)
    summary = audit3.annotation_summary(metadata, label, trace)
    audit3.validate_annotation_inputs(summary, metadata, valid, components, target_mask, catalogue)
    disposition = audit3.retention(plan_sha256, summary["source_family"], summary["disposition"], name, persistence)
    folder, witness_folder = audit3.sample_directories(output, name)
    require(not folder.exists() and not witness_folder.exists(), "Create-only primary/witness samples required")
    primary = witness = None
    if disposition["persist_primary"]:
        primary = write_compact_native_sample(folder, rgb, depth, valid, metadata, instances,
            mapping, catalogue, components, organs, target_mask, label, trace)
    if qualification_witness:
        witness = write_compact_native_sample(witness_folder, rgb, depth, valid, metadata, instances,
            mapping, catalogue, components, organs, target_mask, label, trace)
        require(primary is None or primary == witness, "Same-callback primary/witness binding mismatch")
    available = primary is not None or witness is not None
    return dict(annotation=summary, persistence=disposition, primary=primary, witness=witness,
                pixels_persisted=available, independent_pixel_replay_possible=available)
...
def collect(app, output, plan, base, *, plan_sha256, storage_qualification,
            ...
            persistence=ALL_FRAMES, qualification_witness=False):
    """Explicit opt-in; independently enforce proof admission before native imports."""
    require(not profile_render, 'Compact storage excludes profile_render review PNGs')
    qualification = compact_qualification.check_qualification(
        storage_qualification, expected_sha256=storage_qualification_sha256)
    bindings = implementation_bindings()
    audit3._hash(plan_sha256)
    audit3.validate_plan_mode(plan, persistence, qualification_witness)
    result = _collect(app, output, plan, base, profile_render=profile_render,
    ...
    result.update(compact_native_storage=True, storage_backend=STORAGE_BACKEND,
...
def main(argv=None):
    ...
    try:
        ...
        require(result['compact_implementation_bindings']==compact_bindings
