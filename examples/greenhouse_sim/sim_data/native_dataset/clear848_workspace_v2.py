...
SCHEMA='greenhouse.clear848_kinematic_workspace.v2'
...
class WorkspaceChecker:
    ...
    def solve(self,pose,target,seconds=4.0):
        ...
        try:
            for seed in seeds:
                ...
                # A bounded optimizer can return a feasible iterate without convergence.
                # Certify the returned joint vector directly using independent FK/limits.
                if not np.isfinite(sol.joint_degrees).all():continue
                actual=(model.forward('left',sol.joint_degrees,pose['base'],pose['torso'])@np.r_[pose['probe'],1])[:3]
                if not np.isfinite(actual).all() or float(np.linalg.norm(actual-target))>=.001:continue
                ...
                result.update(workspace_passed=True,status='position_ik_with_joint_limits_and_inter_arm_screen',candidate=dict(
                    **asdict(sol),optimizer_converged=sol.succeeded,independent_fk_position_error_m=float(np.linalg.norm(actual-target)),probe_world_m=actual.tolist(),joint_limit_margin_degrees=float(margin),
    ...
    def check_sample(self,path,expected_sha256):
        ...
        return dict(schema=SCHEMA,sample_path=str(path),sample_sha256=expected_sha256,
            ...
            workspace_admission_basis='independent_FK_residual_below_1mm_joint_limits_and_inter_arm_clearance_not_optimizer_convergence',full_scene_arm_collision_checked=False,approach_path_checked=False,physical_cut_approved=False,
