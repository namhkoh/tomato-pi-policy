...
Creates a new cumulative registry referencing the portable base and storing only
...
SCHEMA='greenhouse.native848_diverse_extension.v5'
REQUEST_SCHEMA='greenhouse.native848_diverse_extension_request.v5'
...
HELPER_MODULES=[Path(__file__).resolve().with_name(name) for name in ('training_export.py','clear_cutpoint_contract.py','capture_contract.py','native848_data_roots_v1.py')]
...
PREVIOUS_WRITERS={'greenhouse.native848_diverse_extension.v4':
    (Path(__file__).resolve().with_name('native848_diverse_extension_v4.py'),
     'b7e0cede8618ae92007b3db0f13549305324cf04458f3fd483c7cf8407705370'),
    'greenhouse.native848_diverse_extension.v2':
    ...
    'greenhouse.native848_diverse_extension.v3':
    (Path(__file__).resolve().with_name('native848_diverse_extension_v3.py'),
     'c5df3c73c2a50c2b7599dc7c7fb2819791a191526db38b1420c3c5f2ec421bc0')}
...
ADAPTERS={
    'greenhouse.native848_diverse_bulk_q3_adapter.v5':('native848_diverse_bulk_adapter_v5.py','04b997f71abfcd75402c1a64bc1f9570c75dc0a0f761e6c32b11e554cdad6759'),
    ...
    'greenhouse.native848_diverse_bulk_q3_adapter.v4':('native848_diverse_bulk_adapter_v4.py','fd244adc37b6170ad5e29417e0d43ac1f18708c7d946f04e6d0fd68632cd9e4b')}
Q4_POSTS={
    'greenhouse.native848_diverse_bulk_q4_background_post.v5':('native848_diverse_bulk_q4_post_v5.py','6cb097014dcfe757715bc9950d08f09823c3d1c28cddf54e76a0277b7f26190f'),
    'greenhouse.native848_diverse_bulk_q4_background_post.v1':('native848_diverse_bulk_q4_post_v1.py','77a1302ab8058115375eb1f8897fc53b4315d4eb3523c4f09d5bbe541ba5d2c9'),
    'greenhouse.native848_diverse_bulk_q4_background_post.v2':('native848_diverse_bulk_q4_post_v2.py',FROZEN_Q4_POST_V2_SHA),
    'greenhouse.native848_diverse_bulk_q4_background_post.v3':('native848_diverse_bulk_q4_post_v3.py','e3c48cee6948208c0ffaa11a5015e3b9f9e7e7b44e86f54159ec32c04649d982'),
    'greenhouse.native848_diverse_bulk_q4_background_post.v4':('native848_diverse_bulk_q4_post_v4.py','2e124c0959e023f30eff3eb72b196f8c43d409cdb961146b29e233d4331c1e5c')}
GROUP_HELPERS={
    'greenhouse.native848_diverse_group_review.v5':('native848_diverse_group_review_v5','d6e53d9e646b27829beab873af8b69e817340d0e7b189369d9de196e33dd3302'),
    'greenhouse.native848_diverse_group_review.v1':('native848_diverse_group_review_v1',FROZEN_GROUP_HELPER_SHA),
    'greenhouse.native848_diverse_group_review.v2':('native848_diverse_group_review_v2',FROZEN_GROUP_HELPER_V2_SHA),
    'greenhouse.native848_diverse_group_review.v3':('native848_diverse_group_review_v3','6745d4a8a973c06693e119c6b10a92297899a5c4b1bcead6e936159e5fd4f944'),
    'greenhouse.native848_diverse_group_review.v4':('native848_diverse_group_review_v4','1d302c604fb036f914d39a08c2e54c429c1dab626d01b9b1a1c9b5082c910c76')}
PARALLEL_KEYS=('parallel_parent_result','parallel_parent_complete','parallel_parent_intent','parallel_slot_ticket','parallel_slot_release')


def group_api_for(schema,request,pins):
    import importlib
    spec=GROUP_HELPERS.get(schema);require(spec is not None,'Unknown reviewed group helper schema')
    module=importlib.import_module('.'+spec[0],__package__);path=Path(module.__file__).resolve()
    require(request['implementation_bindings'].get(str(path))==digest(path)==spec[1],
            'Exact reviewed group helper implementation required')
    pins.add(dict(path=str(path),sha256=spec[1]));return module


def check_group_q4_result(result,request,pins):
    version=result['schema'].rsplit('.v',1)[-1]
    require(version in ('3','4','5'),'Unsupported explicit Q4 group version')
    helper=group_api_for('greenhouse.native848_diverse_group_review.v'+version,request,pins)
    getattr(helper,'verify_post'+version)(result,pins)
    contract=result['effective_visual_review_contract']
    require(contract['label_and_query_bytes_unchanged'] is True,'Review routing may not change labels or queries')
    for row in result['records']:
        require(row['q4_group_review_authorized'] is True
            and row['effective_visual_review_requirement']=='target_view_group_representatives_and_all_flagged'
            and row['legacy_selector_individual_hint_superseded'] is True,
            'New Q4 row must declare its actual review routing')
        trace=read(pins.add(row['artifacts']['query_trace']))
        check_q4_proof(trace['conditional_background'])
        require(row['q4_review_margins']==helper.group_margin.summarize(trace['conditional_background']),
                'Q4 review margins differ from every actual violating probe/pixel')
    return helper
...
def inspect(request_path,request_sha256):
    ...
    validate_storage_policy(request.get('storage_policy'))
    require(request.get('data_root_manifest')==data_roots.root_manifest(),'Exact future data-root marker required')
    for path,expected in data_roots.bindings().items():
        require(request['implementation_bindings'].get(path)==expected,'Unbound shared data roots or root marker')
    ...
    for spec in request['admissions']:
        ...
        is_q4=result['schema'] in Q4_POSTS
        group_q4=is_q4 and result['schema'].endswith(('.v3','.v4','.v5'))
        normalized=result
        if is_q4:
            route='bulk_q4';post2=not result['schema'].endswith('.v1')
            expected_impl,expected_post=Q4_POSTS[result['schema']]
            require(result['implementation']['sha256']==expected_post,'Unreviewed Q4 post-wrapper')
            expected_state=('fresh_original_bulk_background_exception_pending_group_or_individual_review' if group_q4 else
                            'fresh_original_bulk_background_exception_pending_individual_review')
            require(result['state']==expected_state
                ...
                and result['all_candidates_require_individual_visual_review'] is (not group_q4),
                'Separate fresh conditional Q4 result required')
            if group_q4:check_group_q4_result(result,request,pins)
            ...
            data_roots.fresh_native(trial)  # Exact old D and new C native namespaces only.
            ...
            route='bulk_q3'
            modern=result['schema'] in ADAPTERS and not result['schema'].endswith('.v1')
            spec_adapter=ADAPTERS.get(result['schema']);require(spec_adapter is not None,'Unsupported canonical adapter')
            expected_impl=spec_adapter[0]
            ...
            require(result['state']==expected_state and result['implementation']['sha256']==spec_adapter[1]
        ...
        if result['schema'] in ADAPTERS and not result['schema'].endswith('.v1'):
            require(result['implementation']['sha256']==ADAPTERS[result['schema']][1]
        ...
        parallel=normalized['schema'] in ('greenhouse.native848_diverse_bulk_q3_adapter.v4','greenhouse.native848_diverse_bulk_q3_adapter.v5')
        if parallel:
            parallel_version=normalized['schema'].rsplit('.v',1)[-1]
            parallel_helper=group_api_for('greenhouse.native848_diverse_group_review.v'+parallel_version,request,pins)
            getattr(parallel_helper,'verify_adapter'+parallel_version)(normalized,pins)
        ...
        if parallel:evidence_keys+=PARALLEL_KEYS
        ...
        coverage=None
        if group_mode:
            require((route=='bulk_q3' or group_q4) and names,
                    'Only strict Q3 or explicitly reviewed new Q4 group contracts permit group mode')
            group_review=read(pins.add(spec['review']))
            group_api=group_api_for(group_review['schema'],request,pins)
            if group_q4:
                minimum_version=int(result['schema'].rsplit('.v',1)[-1])
                allowed_schemas=tuple('greenhouse.native848_diverse_group_review.v'+str(v) for v in range(minimum_version,6))
                require(group_review['schema'] in allowed_schemas,
                        'Old group helpers cannot override new Q4 review routing')
...
STORAGE_SCHEMA='greenhouse.native848_content_addressed_immutable_storage.v1'


def validate_storage_policy(policy):
    required={'schema','addressing','same_volume','cross_volume','source_immutability',
              'materialized_crops','allow_existing_asset_mutation'}
    require(isinstance(policy,dict) and set(policy)==required,'Explicit complete storage policy required')
    require(policy['schema']==STORAGE_SCHEMA and policy['addressing']=='sha256'
        and policy['same_volume']=='hardlink' and policy['cross_volume'] in ('reject','verified_copy')
        and policy['source_immutability']=='sealed_sources_and_destinations_remain_immutable'
        and policy['materialized_crops'] is True and policy['allow_existing_asset_mutation'] is False,
        'Storage policy must preserve exact immutable bytes and materialized crops')
    return policy


def file_identity(path):
    stat=Path(path).stat()
    return dict(device=stat.st_dev,file_id=stat.st_ino,size=stat.st_size,mtime_ns=stat.st_mtime_ns)


def storage_method(source_device,destination_device,policy):
    validate_storage_policy(policy)
    if source_device==destination_device:return 'hardlink'
    require(policy['cross_volume']=='verified_copy','Cross-volume copy requires explicit verified_copy policy')
    return 'verified_copy'


class ImmutableAssetStore:
    """Create-only, content-addressed aliases of sealed immutable data files.

    Hardlinks share later writes: the contract forbids changing either name.
    Source and destination bytes/identities are verified before final commit.
    """
    def __init__(self,output,policy):
        self.output=data_roots.resolve_data(output,data_roots.CHECKPOINT_ROOTS)
        require(self.output.is_dir(),'Storage output must already exist')
        self.policy=validate_storage_policy(policy)
        self.objects={};self.sources={};self.references=[]
        self.directory=self.output/'assets'/'sha256'
        self.directory.mkdir(parents=True,exist_ok=False)

    def put(self,source,expected,*,role):
        source=data_roots.resolve_data(source)
        require(source.is_file() and not source.is_symlink(),'Regular sealed data source required')
        require(isinstance(expected,str) and len(expected)==64
            and all(c in '0123456789abcdef' for c in expected),'Invalid asset SHA256')
        initial=file_identity(source)
        require(digest(source)==expected and file_identity(source)==initial,'Source changed before asset placement')
        key=str(source)
        if key in self.sources:
            require(self.sources[key]['sha256']==expected and self.sources[key]['identity']==initial,
                    'Previously seen immutable source changed')
        if expected in self.objects:
            item=self.objects[expected];destination=self.output/item['relative_path']
            require(digest(destination)==expected,'Existing content-addressed object changed')
            method='content_reuse'
        else:
            suffix=source.suffix.lower()
            require(len(suffix)<=16 and (not suffix or suffix[1:].isalnum()),'Unsafe asset extension')
            relative=Path('assets')/'sha256'/expected[:2]/(expected+suffix)
            destination=self.output/relative
            destination.parent.mkdir(parents=True,exist_ok=True)
            require(not destination.exists(),'Content-addressed asset overwrite forbidden')
            method=storage_method(initial['device'],destination.parent.stat().st_dev,self.policy)
            if method=='hardlink':
                os.link(source,destination)
                require(os.path.samefile(source,destination),'Hardlink identity differs')
            else:
                with source.open('rb') as src,destination.open('xb') as dst:
                    shutil.copyfileobj(src,dst,2**20)
            require(digest(destination)==expected and digest(source)==expected
                and file_identity(source)==initial,'Asset source/destination changed during placement')
            item=dict(relative_path=relative.as_posix(),sha256=expected,logical_bytes=initial['size'],
                      creation_method=method,source_path=key,source_identity=initial,
                      destination_identity=file_identity(destination))
            self.objects[expected]=item
        require(digest(source)==expected and file_identity(source)==initial,'Source changed after placement')
        self.sources[key]=dict(sha256=expected,identity=initial)
        self.references.append(dict(role=role,source_path=key,sha256=expected,
                                    relative_path=item['relative_path'],method=method))
        return item['relative_path']

    def verify(self):
        for path,item in self.sources.items():
            require(file_identity(path)==item['identity'] and digest(path)==item['sha256'],
                    'Immutable source changed before storage commit')
        for item in self.objects.values():
            destination=self.output/item['relative_path']
            require(file_identity(destination)==item['destination_identity']
                    and digest(destination)==item['sha256'],'Stored object changed before commit')
            if item['creation_method']=='hardlink':
                require(os.path.samefile(destination,item['source_path']),'Stored hardlink identity changed')

    def manifest(self):
        self.verify()
        return dict(schema=STORAGE_SCHEMA,policy=self.policy,objects=list(self.objects.values()),
                    references=self.references,unique_objects=len(self.objects),
                    logical_bytes=sum(x['logical_bytes'] for x in self.objects.values()),
                    hardlinked_bytes=sum(x['logical_bytes'] for x in self.objects.values() if x['creation_method']=='hardlink'),
                    copied_bytes=sum(x['logical_bytes'] for x in self.objects.values() if x['creation_method']=='verified_copy'),
                    hardlinks_are_not_snapshots_against_future_mutation=True,
                    portable_copy_must_materialize_normal_files=True)


def build(request_path,*,request_sha256,output):
    output=data_roots.checkpoint_output(output)
    ...
    try:
        ...
        store=ImmutableAssetStore(output,checked['request']['storage_policy'])
        for spec in checked['request']['admissions']:
            ...
            for item in [spec['result']]+([spec['review']] if spec['review'] is not None else [])+primary:
                relative=store.put(item['path'],item['sha256'],role='primary_evidence')
                portable[relative]=dict(source=item['path'],sha256=item['sha256'])
        for item in checked['group_evidence']:
            relative=store.put(item['path'],item['sha256'],role='group_evidence')
            portable[relative]=dict(source=item['path'],sha256=item['sha256'])
        for row,paths in checked['accepted']:
            ...
            for key,source in paths.items():
                ...
                files[key]=store.put(source,row['artifacts'][key]['sha256'],role='artifact:'+key)
            relative=Path('supplement')/sid/'crop.png'
            (output/relative).parent.mkdir(parents=True,exist_ok=False)
        ...
        status=dict(schema=SCHEMA,starting_counts=dict(train=529,validation=48,test=90),
            **distribution(rows),new_accepted_images_this_batch=len(supplements),
            ...
            new_capture_scope='Every new image passes unchanged automated gates; exact reviewed Q3/Q4 contracts permit deterministic group QA with every flagged case individually inspected; old Q4 contracts remain individual',
            frozen_family_splits=checked['frozen'],training_started=False,training_approved=False)
        write(output/'status.json',status)
        write(output/'storage_manifest.json',store.manifest())
        readme=('# Batched diverse continuation of the fixed529-image starting package\n\n'
            ...
            'Every new image passes its explicit per-image Q3/Q4/workspace gates. Q3 and authorized Q4 group-only members are marked individual_visual_review=false; all representatives and flags are actually inspected. Older Q4 contracts remain individually reviewed. '
            ...
            'Fresh sealed assets use content-addressed paths shared across rows. Same-volume files are hardlinks; cross-volume files require the explicit verified-copy policy. Both source and destination must remain immutable: a hardlink is not a snapshot against later edits. Never edit, compress, or change attributes through either hardlink name. Materialized crops remain ordinary independent files. Portable delivery must materialize ordinary files. Old annotation/review scopes remain unchanged; '
            'New annotation epochs and original Q3 evidence are explicit per row. Primary provenance receipts share the content-addressed store; portable_evidence.json resolves their paths. Transitive replay sources remain pinned external references. '
        ...
        result=dict(schema=SCHEMA,state='diverse_cumulative_batch_with_explicit_review_scopes_complete',
            ...
            output_bindings={name:dict(path=str(output/name),sha256=digest(output/name)) for name in
                ('index.jsonl','supplement_index.jsonl','train_index.jsonl','validation_index.jsonl','test_index.jsonl',
                 'status.json','request.json','portable_evidence.json','exclusions.json','global_exclusion_aliases.json','README.md','storage_manifest.json')},
