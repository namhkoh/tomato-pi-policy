"""Admission-only wrapper for two to four explicitly declared owned filtered-scene pilot workers.

Frozen pilot capture, profile, callback, geometry and static checks are reused.
...
import time
import traceback
from . import native848_pilot_worker_v1 as producer
...
FROZEN_PRODUCER_SHA='54d53f64d69642b128b1201c715de25e38105eb04d123fca84c8a3828ac45fc9'
...
def check_cancellation(control,output):
    require(not (control/'CANCEL_BEFORE_SIMULATIONAPP').exists()
        and not (output/'STOP_AFTER_CURRENT_FRAME').exists(),
        'Coordinator cancelled owned worker')
...
def main(argv=None):
    ...
    require(roots.diagnostic(ticket_path)==ticket_path and sha256(ticket_path)==args.ticket_sha256,'Changed owned ticket')
    root_bindings=roots.bindings()
    ticket=read(ticket_path);control=ticket_path.parent;output=Path(ticket['output']).resolve()
    require(ticket['schema']=='greenhouse.native848_pilot_parallel_ticket.v4' and type(ticket['worker_count']) is int and ticket['worker_count'] in (2,3,4)
        and type(ticket['slot']) is int and 0<=ticket['slot']<ticket['worker_count'] and type(ticket['frames_per_worker']) is int and 1<=ticket['frames_per_worker']<=512
    ...
    require(all(ticket['source_bindings'].get(p)==h for p,h in root_bindings.items()),'Ticket omitted exact data-root contract')
    ...
    try:
        ...
        def check_admission_safety():
            return retirement.check_actual_resources(control,check_stop)
        admission=retirement.inventory(control.parent,coordinator,release['slots'],protected_slot=ticket['slot'],check_stop=check_admission_safety)
        save_json(control/'actual_process_admission.json',admission)
        process=admission['process_inventory']
        ...
        plan=read(plan_path);require(plan['schema']==api.SCHEMA and plan['max_frames']==ticket['frames_per_worker']
            and len(plan['schedule'])==plan['max_frames'],'Actual bounded pilot schedule differs')
        proof=roots.resolve_evidence(ticket['scene_preflight']['path'])
        require(sha256(proof)==ticket['scene_preflight']['sha256'],'Changed CPU scene preflight')
        cpu=read(proof)
        require(cpu['schema']=='greenhouse.native848_pilot_scene_cpu_preflight.v1'
            and cpu['plan_sha256']==ticket['plan']['sha256']
            and Path(cpu['plan_path']).resolve()==plan_path
            and cpu['scene_policy']==plan['pilot_scene_policy']
            and cpu['selected_sample_ids']==plan['selected_sample_ids']
            and cpu['all_selected_pose_screens_passed'] is True,
            'Exact complete filtered-scene preflight required')
        ...
        save_json(output/'request.json',dict(created_utc=datetime.now(timezone.utc).isoformat(),
            ...
            scene_preflight=ticket['scene_preflight'],
        ...
        checked['cpu_scene_preflight_pin']=ticket['scene_preflight']
        for path,pin in {**cpu['source_bindings'],str(proof):ticket['scene_preflight']['sha256']}.items():
            require(path not in checked['source_bindings'] or checked['source_bindings'][path]==pin,
                'CPU source conflicts with immutable capture source')
            checked['source_bindings'][path]=pin
        wrapper_bindings={**ticket['source_bindings'],**admission['source_bindings'],str(ticket_path):args.ticket_sha256,
            **{str((control/name).resolve()):sha256(control/name) for name in
                ('command_identity.json','native_ready.json','release.json','actual_process_admission.json')}}
        ...
        save_json(destination/'failure.json',dict(state='owned_filtered_pilot_parallel_worker_failed',
