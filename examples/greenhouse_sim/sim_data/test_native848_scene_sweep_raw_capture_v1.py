...
import numpy as np
from . import native848_scene_sweep_raw_capture_v1 as p
...
class Fixture(unittest.TestCase):
    def setUp(self):
        ...
        self.proof.update(proposal_only=True,scene_collision_pending=True,native_floor_and_workspace_validation_required=True)
        self.proof.pop('passed');self.proof.pop('success');self.proof.pop('all_selected_pose_screens_passed');self.proof.pop('all_selected_9mm_workspace_checks_passed');self.proof.pop('per_record');self.proof.pop('poses')
...
class ResultTests(Fixture):
    def setUp(self):
        ...
        self.obs=dict(schema=p.OBSERVATION_SCHEMA,observation_id='view0',sample_id='view0',context=self.context_pin,
            ...
            request_evidence=self.events[-1],synchronization=dict(request_index=8,callback_sequence=8),calibration=deepcopy(self.records[0]['calibration']),robot_snapshot=dict(joint_degrees={},robot_root_to_world_usd_row_vectors=self.matrix,camera_to_head_column_vectors=self.matrix,visual_bound_screen=dict(passed=True),whole_robot_collision_checked=True,source_body_orientation_preserved=False,pose_sampling='new_body_orbit_and_head_fk_v1'),actual_workspace_9mm=reach(self.records[0]),geometry=self.records[0]['pilot_9mm_geometry'],**p.FLAGS)
        ...
        self.result=dict(schema=p.RESULT_SCHEMA,request=self.request_pin,native_identity=identity,context=self.context_pin,
            ...
            warmup_requests=deepcopy(self.events[:7]),request_count=8,callback_count=8,workspace_stats=dict(checked_frames=1,source_bindings={self.request['scene_anchor']['path']:self.request['scene_anchor']['sha256']}),**p.FLAGS)
...
class LifecycleTests(Fixture):
    def test_all_collision_holds_cleanly_release_single_product_writer(self):
        ...
        from . import native848_bulk_workspace_v1 as workspace_module
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(patch.object(workspace_module,'BulkWorkspace',return_value=types.SimpleNamespace(bindings={},finish=lambda:dict(checked_frames=0,source_bindings={}))))
        ...
        files=sorted((self.output/'holds').glob('*.json'))
        self.assertEqual(len(files),len(self.records))
        self.assertEqual([json.loads(f.read_text()) for f in files],
            [dict(sample_id=r['sample_id'],reason='whole_robot_scene_collision',screen=dict(passed=False)) for r in self.records])
...
class ImportTests(unittest.TestCase):
    def test_native_admission_import_does_not_load_usd_or_start_app(self):
        ...
        code=('import sys,json;from sim_data import native848_scene_sweep_raw_capture_v1;'
...
class RawRequestTests(Fixture):
    def test_numeric_proposal_honestly_pending(self):
        checked=p.check_request(self.request)
        self.assertTrue(checked['cpu_preflight']['scene_collision_pending'])
        self.assertNotIn('passed',checked['cpu_preflight'])
    def test_prechecked_claim_cannot_replace_pending_contract(self):
        self.proof['scene_collision_pending']=False;self.request['cpu_preflight']=write(self.root/'cpu.json',self.proof)
        with self.assertRaises(ValueError):p.check_request(self.request)

class RawWorkspaceResultTests(ResultTests):
    def test_workspace_hold_allowed(self):
        self.result['holds']=[dict(sample_id='view1',reason='9mm_workspace',workspace=reach(self.records[1],False))]
        self.result['workspace_stats']['checked_frames']=2
        self.assertEqual(self.validate(),self.context)
    def test_native_frame_workspace_wrong_target_rejected(self):
        self.obs['actual_workspace_9mm']['target_id']='foreign/target';self.save_obs()
        with self.assertRaises(ValueError):self.validate()
    def test_native_frame_workspace_wrong_point_rejected(self):
        self.obs['actual_workspace_9mm']['nominal_world_m']=[0,0,2];self.save_obs()
        with self.assertRaises(ValueError):self.validate()
    def test_native_frame_workspace_false_fk_rejected(self):
        self.obs['actual_workspace_9mm']['per_frame_camera_FK_verified']=False;self.save_obs()
        with self.assertRaises(ValueError):self.validate()
    def test_workspace_hold_cannot_borrow_another_target(self):
        self.result['holds']=[dict(sample_id='view1',reason='9mm_workspace',workspace=reach(self.records[0],False))]
        with self.assertRaises(ValueError):self.validate()
    def test_body_orbit_has_honest_provenance(self):
        self.obs['robot_snapshot']['source_body_orientation_preserved']=True;self.save_obs()
        with self.assertRaises(ValueError):self.validate()
