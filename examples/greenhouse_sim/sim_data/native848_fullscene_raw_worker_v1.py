"""Full original-population native848 raw capture; background anatomy is unknown.

All original plants remain. Native capture primitives, seven warmups, reset8,
robot FK, geometry screens, process identity, and telemetry opt-out are retained.
This typed raw output cannot enter filtered-scene annotation/export consumers.
"""
...
import traceback
import numpy as np
from . import native848_fullscene_raw_plan_v1 as api
...
RAW_METADATA=dict(dataset_split='unassigned_pending_full_scene_grouping',
    annotation_coverage='partial_unknown_backdrop_anatomy',no_query9mm_acceptance_pending=True,
    filtered_scene_annotation_export_eligible=False)


def require_raw_scope(value):
    require(all(value.get(k)==v for k,v in RAW_METADATA.items()),'Explicit full-scene raw unknown-anatomy scope required')
    require(value.get('training_approved') is False and value.get('accepted_training_increment')==0,
        'Raw capture must carry zero acceptance and no training approval')
    require('split' not in value,'Raw full scene cannot inherit a filtered dataset split')


def capture(app, output, plan_path, plan, checked):
    import omni.usd
    import omni.replicator.core as rep
    from .native848_fullscene_raw_scene_v1 import prepare_native_scene
    ...
    require_raw_scope(plan)
    cpu=checked['cpu_scene_preflight'];require_raw_scope(cpu)
    require(scene['scene_policy_evidence']['deterministic_census_sha256']==cpu['deterministic_census_sha256'],
            'Native full original plant census differs from exact full-scene CPU preflight')
    ...
    if profile['schema']==api.ORIGINAL_PROFILE_SCHEMA:
        clock_proof=dict(schema='greenhouse.original848_bulk_paused_clock.v1',delta_time_seconds=0,
            scheduler_clock_policy=api.PAUSED_CLOCK_POLICY,original_scene_setup_unchanged=True,full_original_population_retained=True,
    ...
    try:
        ...
        context = dict(**RAW_METADATA, accepted_training_increment=0, schema=api.CONTEXT_SCHEMA, plan_path=str(plan_path.resolve()), plan_sha256=sha256(plan_path),
        ...
        for capture_index, (item, rec) in enumerate(zip(plan['schedule'],records)):
            ...
            try:
                ...
                observation = dict(**RAW_METADATA, accepted_training_increment=0, schema=api.OBSERVATION_SCHEMA,observation_id=name,capture_index=capture_index,
                    ...
                    source_pose_split=checked['capture_split'],capture_role='raw_full_scene_capture',profile=profile['profile'],
                ...
                print('FULLRAW848_QUEUED',capture_index,name,round(request['elapsed_seconds'],4),flush=True)
        ...
        manifest = dict(**RAW_METADATA,schema='greenhouse.native848_fullscene_raw_manifest.v1',context_path=str(context_path.resolve()),
        ...
        require_raw_scope(manifest)
        save_json(output/'batch_manifest.json',manifest)
        return dict(**RAW_METADATA,schema='greenhouse.native848_fullscene_raw_capture.v1',state=api.RESULT_STATE,manifest_path=str((output/'batch_manifest.json').resolve()),
...
def native_command(args):
    return ['D:/isaac-sim-6.0.1/python.bat','-B','-u','-m','sim_data.native848_fullscene_raw_worker_v1',
...
def record_native_identity(args):
    ...
    return dict(schema='greenhouse.native848_fullscene_raw_worker_identity.v1',owner_identity=owner,
...
def validate_native_identity(value,command,*,owner_identity=None):
    require(value['schema']=='greenhouse.native848_fullscene_raw_worker_identity.v1'
        and value['same_snapshot_identity_checks'] is True and value['command']==command
        and command[1:5]==['-B','-u','-m','sim_data.native848_fullscene_raw_worker_v1'],
        'Exact fullscene raw native module identity required')
...
def runtime_evidence(phase,config,native_pin):
    ...
    return dict(schema='greenhouse.native848_fullscene_raw_runtime_evidence.v1',phase=phase,
...
def validate_optout_capture(capture,value,context):
    ...
    require_raw_scope(value);require_raw_scope(context)
    ...
    for phase in ('start','end'):
        ...
        require(evidence['schema']=='greenhouse.native848_fullscene_raw_runtime_evidence.v1'
...
def main(argv=None):
    ...
    require(plan['schema']==api.SCHEMA and not output.exists(),'New full-scene raw output required')
    require_raw_scope(plan)
    ...
    try:
        ...
        cpu=read_json(args.scene_preflight);require_raw_scope(cpu)
        require(cpu['schema']=='greenhouse.native848_fullscene_raw_cpu_preflight.v1'
        ...
        save_json(output/'failure.json',dict(state='native848_fullscene_raw_failed',error=traceback.format_exc(),
