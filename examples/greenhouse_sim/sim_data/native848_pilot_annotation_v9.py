...
from . import native848_pilot_worker_v2 as optout_worker
...
from . import native848_pilot_failed_parent_recovery_v2 as latest_recovery
from . import native848_pilot_failed_parent_recovery_v3 as failed68_recovery
from . import native848_pilot_failed_parent_recovery_v4 as failed64_recovery
from . import native848_pilot_parallel_predecessor_v4 as parallel_predecessor
...
SCHEMA='greenhouse.native848_filtered_pilot_9mm_evaluation.v9'
OWNER_SCHEMAS={
    ...
    'greenhouse.native848_unique9mm_pilot_owner.v3':('run_native848_pilot_v3.py','2e763ef040ac8c4ed120e3432e8b634fc2f82beca0703b054009f6d688a769a0'),
    'greenhouse.native848_unique9mm_pilot_owner.v4':('run_native848_pilot_v4.py','fb5a20ee16fe36036b29d33e768521d779fd5fffc6012ee826217b3af185b69e'),
    'greenhouse.native848_unique9mm_pilot_owner.v5':('run_native848_pilot_v5.py','cfb260f8f620adf1e76ba87691ef596fd3433b331c9dff69ef268f0e8377aeb4'),
    'greenhouse.native848_unique9mm_pilot_owner.v6':('run_native848_pilot_v6.py','3be9ce62584eea2dbe2c78bf7ad9361c3404dea80553032d8c25127f9355365a')}
AMBIGUITY_SHA256='c42bca181ca07af4446c0ff5c24f14126573bc39dc8eeeb609a8e835fd9ac2fe'
ROOT=Path(__file__).resolve().parents[3]


def check_recovery(spec):
    require(set(spec)=={'path','sha256'}, 'Exact typed recovery pin required')
    path=latest_recovery.roots.resolve_evidence(spec['path'])
    require(sha256(path)==spec['sha256'], 'Changed typed recovery evidence')
    value=read_json(path)
    implementations={recovery.SCHEMA:recovery,latest_recovery.SCHEMA:latest_recovery,failed68_recovery.SCHEMA:failed68_recovery,failed64_recovery.SCHEMA:failed64_recovery}
    require(value.get('schema') in implementations, 'Unknown finite recovery schema')
    return implementations[value['schema']].check(spec)
...
def authenticate_owner(trial,owner,result_sha256,read,bound,parent_metadata,parent_recovery=None):
    ...
    if parent_recovery is not None:
        ...
        proof=check_recovery(parent_recovery)
    ...
    if schema in OWNER_SCHEMAS:
        ...
        bound(path,pin)
        if schema=='greenhouse.native848_unique9mm_pilot_owner.v6':
            ...
            require(actual['command_identity']['ProcessId']==launch['pid'],'Actual opt-out command PID differs')
            for p,h in proof['source_bindings'].items():bound(p,h)
    else:
...
def run(trial,result_sha256,output,*,parent_metadata=None,parent_recovery=None):
    ...
    for path,h in {**checker.bindings,str(Path(__file__).resolve()):sha256(__file__),str(Path(labels.__file__).resolve()):sha256(labels.__file__),str(Path(ambiguity.__file__).resolve()):AMBIGUITY_SHA256,str(Path(parallel_predecessor.__file__).resolve()):sha256(parallel_predecessor.__file__),str(Path(recovery.__file__).resolve()):sha256(recovery.__file__),str(Path(latest_recovery.__file__).resolve()):sha256(latest_recovery.__file__)}.items():
