...
def capture(app, output, plan_path, plan, checked):
    ...
    check_cancellation(output.parent,output)
    ...
    try:
        ...
        for capture_index, (item, rec) in enumerate(zip(plan['schedule'],records)):
            ...
            try:
                if cancellation_requested(output.parent,output):
                ...
                observation = dict(**population_metadata(checked['capture_split'],native=True), accepted_training_increment=0, schema=api.OBSERVATION_SCHEMA,observation_id=name,capture_index=capture_index,
                    ...
                    source_pose_split=checked['capture_split'],capture_role='fully_labeled_qualification_candidate',profile=profile['profile'],
        ...
        manifest = dict(**population_metadata(checked['capture_split'],native=True),schema='greenhouse.native848_fully_labeled_manifest.v1',context_path=str(context_path.resolve()),
        ...
        return dict(**population_metadata(checked['capture_split'],native=True),schema='greenhouse.native848_fully_labeled_capture.v1',state=api.RESULT_STATE,manifest_path=str((output/'batch_manifest.json').resolve()),
...
def optout_bindings():
    ...
    bindings[str(Path(__file__).with_name('native848_fully_labeled_worker_v3.py').resolve())]='8882e3d0c73d26ce2ffc2fa49d801fb8c2e684718b4f2581a31180a4f5099abe'
    verify_bindings(bindings);return bindings


def native_command(args):
    return ['D:/isaac-sim-6.0.1/python.bat','-B','-u','-m','sim_data.native848_fully_labeled_sibling_worker_v1',
        '--ticket',str(args.ticket.resolve()),'--ticket-sha256',args.ticket_sha256]
...
def record_native_identity(args):
    ...
    return dict(schema='greenhouse.native848_fully_labeled_sibling_identity.v1',owner_identity=owner,
...
def validate_native_identity(value,command,*,owner_identity=None):
    require(value['schema']=='greenhouse.native848_fully_labeled_sibling_identity.v1'
        and value['same_snapshot_identity_checks'] is True and value['command']==command
        and command[1:5]==['-B','-u','-m','sim_data.native848_fully_labeled_sibling_worker_v1'],
...
def runtime_evidence(phase,config,native_pin):
    ...
    return dict(schema='greenhouse.native848_fully_labeled_sibling_runtime.v1',phase=phase,
...
def validate_optout_capture(capture,value,context):
    ...
    for phase in ('start','end'):
        ...
        require(evidence['schema']=='greenhouse.native848_fully_labeled_sibling_runtime.v1'
...
TICKET_SCHEMA='greenhouse.native848_fully_labeled_two_ticket.v1'

def cancellation_requested(control,output):
    return (control/'CANCEL_BEFORE_SIMULATIONAPP').exists() or (output/'STOP_AFTER_CURRENT_FRAME').exists()


def check_cancellation(control,output):
    require(not cancellation_requested(control,output),'Coordinator canceled exact owned full-population worker')


def await_json(path,control,output,timeout_seconds=120):
    end=time.monotonic()+timeout_seconds
    while not path.exists():
        check_cancellation(control,output)
        require(time.monotonic()<end,'Bounded native handshake timed out')
        time.sleep(.1)
    check_cancellation(control,output)
...
def main(argv=None):
    ...
    args=parser.parse_args(argv);args.ticket=diagnostic(args.ticket)
    require(sha256(args.ticket)==args.ticket_sha256,'Exact owned ticket changed')
    ticket=read_json(args.ticket);control=args.ticket.parent;output=Path(ticket['output']).resolve()
    require(ticket['schema']==TICKET_SCHEMA and ticket['worker_count']==2 and type(ticket['slot']) is int
        and ticket['slot'] in (0,1) and output==control/'capture' and not output.exists(),
        'Exactly two finite full-population tickets with fresh distinct output required')
    require(sha256(__file__)==ticket['worker_sha256'] and sha256(frozen.__file__)==FROZEN_CAPTURE_SHA256,
        'Reviewed sibling worker or frozen capture changed')
    verify_bindings(ticket['source_bindings'])
    args.plan=Path(ticket['plan']['path']).resolve();args.plan_sha256=ticket['plan']['sha256']
    args.scene_preflight=Path(ticket['scene_preflight']['path']).resolve();args.scene_preflight_sha256=ticket['scene_preflight']['sha256']
    args.output=output
    require(sha256(args.plan)==args.plan_sha256 and sha256(args.scene_preflight)==args.scene_preflight_sha256,'Plan/CPU changed')
    plan=read_json(args.plan);require_scope(plan,native=False)
    require(plan['schema']==api.SCHEMA and plan['max_frames']==ticket['frames_per_worker']
        and 1<=plan['max_frames']<=64 and len(plan['schedule'])==plan['max_frames'],'Exact bounded full-scene plan required')
    app=None;succeeded=False;simulation_app_construction_started=False
    try:
        command=await_json(control/'command_identity.json',control,output)
        ...
        require(identity_gate.same_identity(identity['owner_identity'],ticket['coordinator_identity'])
            and identity_gate.same_identity(identity['command_identity'],command['identity'])
            and identity['command']==command['command'],'Exact current owned chain differs')
        save_json(control/'native_ready.json',dict(slot=ticket['slot'],native_identity=identity['native_identity'],
            command_identity=identity['command_identity'],ticket_sha256=args.ticket_sha256,
            simulation_app_started=False,created_utc=datetime.now(timezone.utc).isoformat()))
        release=await_json(control/'release.json',control,output)
        require(release['slot']==ticket['slot'] and release['worker_count']==2 and release['ticket_sha256']==args.ticket_sha256
            and release['coordinator_identity']==ticket['coordinator_identity'] and release['resource_admission']['allowed'] is True,
            'Exact reviewed fresh resource release required')
        own=next(s for s in release['slots'] if s['slot']==ticket['slot'])
        require(own['native_identity']==identity['native_identity'] and own['command_identity']==identity['command_identity'],
            'Release names another native chain')
        rows=identity_gate.snapshot()
        process=identity_gate.inventory(identity_gate.owned_allowlist(ticket['coordinator_identity'],release['slots']),rows=rows)
        require(process['no_blockers_observed'] is True and not process['blockers'],'Unknown process at actual native admission')
        from sim_physics.host_memory import preflight
        memory=preflight();gpu=identity_gate.gpu_snapshot();disk=shutil.disk_usage(control).free
        require(memory['allowed'] and memory['commit_headroom_bytes']>=20*2**30 and gpu['free_mib']>=4096 and disk>=60*2**30,
            'Actual pre-SimulationApp reserve failed')
        check_cancellation(control,output)
        output.mkdir()
        save_json(output/'request.json',dict(created_utc=datetime.now(timezone.utc).isoformat(),plan_path=str(args.plan),
            plan_sha256=args.plan_sha256,scene_preflight=ticket['scene_preflight'],ticket_path=str(args.ticket),ticket_sha256=args.ticket_sha256,
            host_memory_preflight=memory,gpu=gpu,available_disk_bytes=disk,process_admission=process,
            process_snapshot=rows,automatic_retries=False,training_approved=False))
        ...
        require(profile['schema']==api.ORIGINAL_PROFILE_SCHEMA,'Only original reset8 production profile admitted')
        ...
        save_json(control/'simulation_app_starting.json',dict(ticket_sha256=args.ticket_sha256,native_identity=identity['native_identity'],
            simulation_app_construction_started=True,training_approved=False))
        app_constructor_started=time.perf_counter();app=SimulationApp(config)
        ...
        plan_check_started=time.perf_counter();checked=api.check(plan)
        ...
        require(cpu['schema']=='greenhouse.native848_fully_labeled_cpu_preflight.v3'
            ...
            and cpu['all_selected_pose_screens_passed'] is True and cpu['selected_sample_ids']==plan['selected_sample_ids']
            and cpu['pose_anchor_sources']==checked['pose_anchor_sources'],'Exact selected CPU scene preflight required')
        checked['cpu_scene_preflight']=cpu;checked['cpu_scene_preflight_pin']=ticket['scene_preflight']
        additions={**cpu['source_bindings'],str(args.scene_preflight):args.scene_preflight_sha256,
            **startup_bindings,**ticket['source_bindings'],str(args.ticket):args.ticket_sha256,
            **{str((control/name).resolve()):sha256(control/name) for name in ('command_identity.json','native_ready.json','release.json')},
            **{str((output/name).resolve()):sha256(output/name) for name in ('native_identity.json','telemetry_optout_start.json')}}
        for path,digest in additions.items():
            require(path not in checked['source_bindings'] or checked['source_bindings'][path]==digest,'Exact startup source conflict')
            checked['source_bindings'][path]=digest
        verify_bindings(checked['source_bindings']);check_cancellation(control,output)
        ...
        result.update(telemetry_optout_contract=OPT_OUT_CONTRACT,native_identity=native_pin,plan_sha256=args.plan_sha256,
            bounded_two_worker_slot=ticket['slot'],ticket=dict(path=str(args.ticket),sha256=args.ticket_sha256),
        ...
        save_json(destination/'failure.json',dict(state='native848_fully_labeled_two_worker_failed',error=traceback.format_exc(),
            exception_type=type(exc).__name__,simulation_app_construction_started=simulation_app_construction_started,
            ticket_sha256=args.ticket_sha256,automatic_retries=False,training_approved=False,accepted_training_increment=0))
        raise
    finally:
