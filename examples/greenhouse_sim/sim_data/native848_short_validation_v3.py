...
SCHEMA = 'greenhouse.native848_short_validation_plan.v3'
SAMPLE = 'greenhouse.native848_short_validation_sample.v3'
STATE = 'native848_short_validation_complete_diagnostic_only'
BUDGETS = [1, 2, 4]
A, B = 'SubStem_42_view_002', 'SubStem_44_view_002'
PROFILE = 'reset_each_snapshot_after_authored_pose_no_hidden_settling.v3'
...
def recording_writer(rep,unrequested_folder):
    ...
    class DiagnosticWriter(rep.Writer):
        def __init__(self):
            self.version='native848-short-validation-v3'
...
def full_scene(app, output, plan, scene_name, loaded):
    ...
    workspace=WorkspaceChecker();monitor=StaticSceneMonitor(stage,robot['root'])
    product=rep.create.render_product(HEAD_CAMERA,LEGACY_RESOLUTION)
    writer=recording_writer(rep,output/'unrequested_callbacks');writer.attach([product])
    ...
    try:
        for item in plan['schedules'][scene_name]:
            ...
            print('SHORT848_V3',scene_name,item['name'],receipt.get('capture_valid',False),receipt['passed_actual_automated_criteria'],flush=True)
    finally:
        monitor.close();geometry.close();workspace.finish();writer.detach();product.destroy()
    require(source_hashes(stage)==pins,'Source asset changed');verify_bindings(cache['source_bindings'])
...
def audit(output,plan):
    ...
    actual_files={str(p.resolve()) for p in output.rglob('*') if p.is_file() and p.name not in ('result.json','evidence_bindings.json','audit_v3.json')}
...
def cpu_tests():
    ...
    import tempfile
    from .native_instances import normalize_payload
    with tempfile.TemporaryDirectory(prefix='short848_v3_cpu_',dir=D) as temporary:
...
def main(argv=None):
    ...
    if args.action=='audit':
        ...
        require(not (output/'audit_v3.json').exists(),'Create-only audit required')
        write_json(output/'audit_v3.json',report);return
