...
ANNOTATION_EPOCH = 'greenhouse.native848_all_petiole_9mm.v1'
NOMINAL_ARC_M = .009
...
def evaluate_target(metadata, rgb, depth, valid, components, catalogue, entry, workspace_checker):
    ...
    proximal = [probe(p, own if p['arc_m'] >= .008 else own | {parent['component_index']}) for p in geo['proximal']]
    junction = [probe(p, own if p['arc_m'] >= .008 else own | {parent['component_index']}) for p in geo['junction_to_cut']]
    ...
    base['parent_context'] = dict(visible_parent_pixels=count, cut_to_parent_distance_px=distance,
