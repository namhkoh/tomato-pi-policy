...
SCHEMA='greenhouse.native848_unique_petiole_frame_dataset.v1'
...
def validate_source_identity(spec,metadata,annotation,target):
    coverage_spec=annotation['target_census']['full_scene_coverage']
    coverage=json.loads(read_pin({k:coverage_spec[k] for k in ('path','sha256')}).read_text())
    ...
    require(context['schema']=='greenhouse.native848_unique9mm_pilot_context.v1'
        and context['split']==split and context['scene_policy']['capture_split']==split,
    ...
    require(observation['schema']=='greenhouse.native848_unique9mm_pilot_observation.v1'
        and observation['observation_id']==metadata['sample_id']==annotation['frame_id']
        and observation['split']==split and observation['calibration']==metadata['calibration']
    ...
    return dict(source_family=family,split=split,scene_source_families=families,context=coverage['context'])
...
def materialize_frames(frames,output,*,root_authorization):
    ...
    for spec in frames:
        ...
        checked.append((spec,paths,meta,annotation,rgb,depth,valid,answer,context,answer_audit,identity))
    ...
    preview=[dict(frame_id=x[3]['frame_id'],decoded_rgb_sha256=hashlib.sha256(x[4].tobytes()).hexdigest(),
        conservative_camera_signature=x[0]['conservative_camera_signature'],source_family=x[10]['source_family'],split=x[10]['split'],scene_source_families=x[10]['scene_source_families'],answer=x[7]) for x in checked]
    counts=unique_counts(preview);output.mkdir();records=[]
    try:
        for base,item in zip(preview,checked):
            spec,paths,meta,annotation,rgb,depth,valid,answer,context,answer_audit,identity=item
        ...
        manifest=dict(schema=SCHEMA,task_id=TASK,annotation_epoch=EPOCH,**counts,nominal_arc_m=.009,input_query=False,input_target_id=False,
