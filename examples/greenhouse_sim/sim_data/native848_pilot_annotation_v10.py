...
from . import native848_fully_labeled_plan_v1 as plan_api
from . import native848_fully_labeled_worker_v1 as optout_worker
...
from . import native848_fully_labeled_coverage_v1 as full_coverage
...
SCHEMA='greenhouse.native848_fully_labeled_9mm_evaluation.v10'
OWNER_SCHEMA='greenhouse.native848_fully_labeled_owner.v1'
OWNER_SHA256='2b583788f34e101e856bec8baa3bbb1dd26f74968522f2a831fe637dd3af7f2e'
WORKER_SHA256='749c9239729c82254f076ddbc2cfccd0d14e3cc09c0b3f576fb00d2a3ca50f9d'
COVERAGE_SHA256='f03c2eea1df3aa7f3f4bb7004e31b177cafd495e2da517db11538a940b532349'
...
def authenticate_owner(trial,owner,result_sha256,read,bound,parent_metadata,parent_recovery=None):
    ...
    require(owner['schema']==OWNER_SCHEMA
        and owner['state']=='owned_fully_labeled_population_capture_exited_pending_annotation_and_review'
    ...
    path=ROOT/'data/sim_data/diagnostics/collection_20k_848_20260916_v1/run_native848_fully_labeled_serial_v1.py'
    require(owner['source_bindings'].get(str(path.resolve()))==OWNER_SHA256,'Unknown full-population owner implementation')
    bound(path,OWNER_SHA256)
    require(owner['source_bindings'].get(str(Path(optout_worker.__file__).resolve()))==WORKER_SHA256==sha256(optout_worker.__file__),
...
def finite_indices(total,start_index,frame_count):
    require(type(total) is int and total>=0 and type(start_index) is int and type(frame_count) is int
        and 1<=frame_count<=64 and (0<=start_index<total or total==start_index==0),
        'Explicit finite committed-frame chunk of at most64 required')
    return list(range(start_index,min(total,start_index+frame_count)))


def run(trial,result_sha256,output,*,parent_metadata=None,parent_recovery=None,start_index=0,frame_count=64):
    ...
    for path,h in {**checker.bindings,str(Path(__file__).resolve()):sha256(__file__),str(Path(labels.__file__).resolve()):sha256(labels.__file__),str(Path(ambiguity.__file__).resolve()):AMBIGUITY_SHA256,str(Path(full_coverage.__file__).resolve()):COVERAGE_SHA256}.items():
...
if __name__=='__main__':
    ...
    parser.add_argument('--start-index',type=int,default=0);parser.add_argument('--frame-count',type=int,default=64)
