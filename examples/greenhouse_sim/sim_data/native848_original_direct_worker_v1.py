...
def capture(app, output, plan, cache, anchor):
    ...
    from .static_render import reference_payload
    ...
    # The anchor's generated assets are validated as source dependencies only.
    # The reconstructed original stage is captured without plant substitution.
    ...
    variant = scene['original_variant']
    worlds = {}
    for name in plan['selected_sample_ids']:
        row = by_id[name]['source_row']
        world = target_world_geometry(stage, variant, row)
        expected = world_from_row(anchor, row)
        require(all(np.allclose(world[k], expected[k], atol=1e-9, rtol=0) for k in world),
                'Original world geometry differs')
        worlds[name] = world
    ...
    try:
        for name in plan['selected_sample_ids']:
            ...
            record = by_id[name]
            row, world = record['source_row'], worlds[name]
            ...
            require(not np.isin(instances, generated_ids).any(), 'Generated geometry entered original frame')
            metadata = dict(schema_version=api.SAMPLE_SCHEMA, sample_id=name,
                ...
                direct_camera=dict(pose_cache_sha256=plan['cache_sha256'], cached_sample_id=name,
                    anchor_pair_plan_sha256=record['anchor_pair_plan_sha256'], runtime_pose_search=False,
    ...
    return dict(state=api.RESULT_STATE, resolution=[848, 408], profile=api.PROFILE,
        ...
        short_profile_comparisons_passed=False, established56_every_frame=True,
...
def main(argv=None):
    ...
    try:
        ...
        result = capture(app, output, plan, cache, anchor)
        ...
        write_json(output / 'failure.json', dict(state='original848_direct_batch_failed',
