...
EVALUATION_SCHEMA = "greenhouse.native848_persistent_experimental_evaluation.v1"
...
def evaluate_segment(capturepath,output):
    ...
    profile=producer.diagnostic.actual_profile(authority['scheduled'],candidates['profile'],profile)
    ...
    require(set(r['observation_id'] for r in result['frames'])<=set(authority['scheduled']['selected_sample_ids']),'Wrong experimental cameras')
    ...
    try:
        ...
        value=dict(schema=EVALUATION_SCHEMA,source_capture=str(capture),
            ...
            frames_evaluated=len(summaries),experimental_profile=True,production_profile_qualification=False,
            capture_purpose=authority['scheduled']['purpose'],export_prohibited=authority['scheduled']['purpose']==diagnostic.CONTROL,
            paired_control_reference=authority['scheduled']['paired_control_reference'],
...
def validate_registry(request_pin, identity_pin, batch, scheduled, registered, values, contexts, observations, sources, profiles, producer):
    ...
    require(batch['schema']==producer.RESULT_SCHEMA and batch['request']==request_pin
        ...
        and batch['experimental_profile'] is True,
    ...
    for i,(spec,entry,value,context,obslist,source,profile) in enumerate(zip(scheduled,registered,values,contexts,observations,sources,profiles)):
        sid=f's{i:03d}'
        diagnostic.validate_evidence(spec,profile,context,value,obslist)
...
def authenticate_batch(capture,read,local,producer):
    ...
    require(batch['experimental_profile_policy']==request['experimental_profile_policy'] and complete['experimental_profile'] is True,
        'Batch diagnostic profile or owner scope differs')
    ...
    for i,(spec,entry,(source,candidates)) in enumerate(zip(request['segments'],batch['segments'],sources)):
        ...
        for receipt in value['frames']:
            ...
            obslist.append(obs)
        values.append(value);contexts.append(context);observations.append(obslist);actual=producer.diagnostic.actual_profile(spec,candidates['profile'],read(candidates['profile']));profiles.append(actual)
        diagnostic.validate_evidence(spec,actual,context,value,obslist)
    ...
    return dict(complete=complete,segment_result=value,source_request=source,candidates=candidates,source_bindings=bindings,scheduled=request['segments'][i],
        public=dict(batch_owner_complete=local(trial/'owner_complete.json'),batch_result=complete['result'],batch_request=complete['request'],
            ...
            all_batch_segments_and_global_clocks_authenticated=True,experimental_profile=True,
            actual_experimental_profile=contexts[i]['actual_experimental_profile'],experimental_profile_policy=request['experimental_profile_policy'],
            capture_purpose=request['segments'][i]['purpose'],paired_control_reference=request['segments'][i]['paired_control_reference'],
            export_prohibited=request['segments'][i]['purpose']==diagnostic.CONTROL))
