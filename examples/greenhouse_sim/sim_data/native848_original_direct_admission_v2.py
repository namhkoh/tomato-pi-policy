...
def run(trial, *, result_sha256, output):
    ...
    require(sha256(trial/'result.json') == result_sha256 and not (trial/'failure.json').exists()
        and receipt['state'] == 'owned_original848_reference_direct_completed_and_audited'
    ...
    require(launch['plan_sha256'] == intent['plan_sha256'] == receipt['plan_sha256']
        and 'sim_data.native848_original_direct_worker_v2' in launch['command']
    ...
    try:
        for record in actual['records']:
            ...
            source_row = next(r['source_row'] for r in cache['records'] if r['sample_id'] == name)
            ...
            out = dict(sample_id=name, target_id=label['target_id'], source_target=source_row['target_id'],
        ...
        write_json(output/'result.json', dict(state='original848_reference_direct_labels_trace_workspace_complete_pending_review_and_global_admission',
