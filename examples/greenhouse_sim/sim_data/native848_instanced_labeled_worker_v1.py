...
import traceback
import numpy as np
from . import native848_instanced_labeled_plan_v1 as api
...
def population_metadata(split,*,native):
    ...
    return dict(dataset_split=split,annotation_coverage=VERIFIED_COVERAGE if native else PENDING_COVERAGE,
        ...
        filtered_scene_annotation_export_eligible=False,diagnostic_equivalence_replay=True,training_export_eligible=False)
...
def require_native_census(census,cpu):
    ...
    require(census['schema']=='greenhouse.native848_instanced_labeled_census.v1'
...
def capture(app, output, plan_path, plan, checked):
    ...
    from .native848_instanced_labeled_scene_v1 import prepare_native_scene
    ...
    from .native848_instanced_static_guard_v1 import StaticSceneMonitor
    ...
    try:
        ...
        for capture_index, (item, rec) in enumerate(zip(plan['schedule'],records)):
            ...
            try:
                ...
                observation = dict(**population_metadata(checked['capture_split'],native=True), accepted_training_increment=0, schema=api.OBSERVATION_SCHEMA,observation_id=name,capture_index=capture_index,
                    ...
                    source_pose_split=checked['capture_split'],capture_role='instancing_equivalence_diagnostic',profile=profile['profile'],
        ...
        manifest = dict(**population_metadata(checked['capture_split'],native=True),schema='greenhouse.native848_instanced_labeled_manifest.v1',context_path=str(context_path.resolve()),
        ...
        return dict(**population_metadata(checked['capture_split'],native=True),schema='greenhouse.native848_instanced_labeled_capture.v1',state=api.RESULT_STATE,manifest_path=str((output/'batch_manifest.json').resolve()),
...
def native_command(args):
    return ['D:/isaac-sim-6.0.1/python.bat','-B','-u','-m','sim_data.native848_instanced_labeled_worker_v1',
...
def record_native_identity(args):
    ...
    return dict(schema='greenhouse.native848_instanced_labeled_worker_identity.v1',owner_identity=owner,
...
def validate_native_identity(value,command,*,owner_identity=None):
    require(value['schema']=='greenhouse.native848_instanced_labeled_worker_identity.v1'
        and value['same_snapshot_identity_checks'] is True and value['command']==command
        and command[1:5]==['-B','-u','-m','sim_data.native848_instanced_labeled_worker_v1'],
...
def runtime_evidence(phase,config,native_pin):
    ...
    return dict(schema='greenhouse.native848_instanced_labeled_runtime_evidence.v1',phase=phase,
...
def validate_optout_capture(capture,value,context):
    ...
    for phase in ('start','end'):
        ...
        require(evidence['schema']=='greenhouse.native848_instanced_labeled_runtime_evidence.v1'
...
def main(argv=None):
    ...
    try:
        ...
        require(cpu['schema']=='greenhouse.native848_instanced_labeled_cpu_preflight.v1'
            and cpu['plan_sha256']==args.plan_sha256 and cpu['scene_policy']==checked['scene_policy']
            and cpu['all_selected_pose_screens_passed'] is True and cpu['all_geometry_parity_passed'] is True
