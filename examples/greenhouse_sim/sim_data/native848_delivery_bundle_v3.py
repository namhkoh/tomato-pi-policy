"""Reference-only delivery for original, generated, and farmerge native9mm datasets.
...
from . import native848_generated_farmerge_dataset_v1 as generated_farmerge
from . import native848_scene_sweep_dataset_v1 as sweep

SCHEMA='greenhouse.native848_reference_delivery_bundle.v3'
ENTRY_SCHEMA='greenhouse.native848_reference_delivery_entry.v3'
LOADERS={'original':original,'generated':generated,'generated_farmerge':generated_farmerge,'scene_sweep':sweep}
LOADER_PINS={'original':'f3934b4f89b717866e58415c6700184d77ec497b6886005ff0c7e642ea41414a',
 ...
 'generated_farmerge':'999a14958646a72bdf21243201645ccf34e9fe2274018272a3e5de6f32e61aae','scene_sweep':'e9231588197e0f96d2ff67555805493f7e54af27339a6cba7a0d0eccde7a8777'}
MANIFEST_SCHEMAS={'original':'greenhouse.native848_fully_labeled_verified_checkpoint_union.v6',
 ...
 'generated_farmerge':'greenhouse.native848_reviewed_generated_farmerge_supplement.v1','scene_sweep':sweep.SCHEMA}
COMPLETE_STATES={'original':'accepted_fully_populated_checkpoint_union',
 'generated':'complete_reviewed_generated_supplement',
 'generated_farmerge':'complete_reviewed_generated_farmerge_supplement_pending_union','scene_sweep':sweep.STATE}
...
def _component(spec):
    ...
    require(result['state']==COMPLETE_STATES[kind],
    ...
    if kind=='scene_sweep':
        require(result['schema']==sweep.SCHEMA and manifest['implementation_sha256']==LOADER_PINS[kind]
            and manifest['target_hint_used_to_select_answer'] is False and manifest['input_query'] is False,
            'Explicit reviewed scene-sweep source required')
    if kind=='generated_farmerge':
        require(result['schema']==MANIFEST_SCHEMAS[kind]
            and manifest['implementation_sha256']==LOADER_PINS[kind]
            and manifest['farmerge_representation_preserves_all_background_shapes_materials'] is True
            and manifest['individual_coarse_component_pixel_visibility_claimed'] is False
            and manifest['original_background_instances']==143 and manifest['generated_foreground_instances']==1
            and manifest['annotation_recomputed'] is False and manifest['model_inputs_loaded_after_copy'] is True,
            'Completed explicit full-population farmerge export required')
    provenance_pins=set()
    ...
    for row in rows:
        ...
        if kind=='scene_sweep':
            require(row['split']=='train' and row['source_geometry_modified'] is False
                and row['new_independent_source_family'] is False
                and row['source_provenance']['evaluation']==manifest['source_evaluation']
                and row['source_provenance']['review']==manifest['actual_review']
                and row['source_provenance']['capture_kind'] in ('greenhouse.native848_scene_sweep_context.v1','greenhouse.native848_scene_sweep_raw_context.v1'),
                'Scene-sweep donor/capture/review provenance differs')
        if kind=='generated_farmerge':
            require(row['split']=='train' and row['source_provenance']['render_census']==manifest['render_census']
                and row['source_provenance']['farmerge_recipe']==manifest['farmerge_recipe'],
                'Fast row render authority or TRAIN lineage differs')
            for key in ('render_census','farmerge_recipe','production_profile','source_pose_profile'):
                source=row['source_provenance'][key]
                require(set(source)=={'path','sha256'},'Exact fast render provenance pin required')
                stamp=(source['path'],source['sha256'])
                if stamp not in provenance_pins:original.read_pin(source);provenance_pins.add(stamp)
    return root,rows
...
def entry_for(component_id,kind,row_number,row):
    ...
    donor=row['source_family'];modified=kind in ('generated','generated_farmerge')
    if modified:
...
def create_bundle(components,output,*,expected_images):
    ...
    try:
        ...
        manifest=dict(schema=SCHEMA,task_id=original.TASK,annotation_epoch=original.EPOCH,
            ...
            counts=total,loader='sim_data.native848_delivery_bundle_v3.DeliveryDataset',loader_sha256=original.digest(__file__),
        ...
        text+='Detailed target and geometry counts are in `manifest.json`. Generated geometry IDs retain their donor split and do not create independent donors. Modified geometries are limited donor-derived subtree augmentations; broad morphology diversity is not claimed.\n\n'
        text+='## Files and storage\n\n`index.jsonl` is the normalized training index; split indexes select TRAIN, validation and TEST. The manifest pins each source release and dispatches its distinct schema explicitly. Farmerge sources retain their typed render provenance and positive coarse-geometry exclusion evidence in the unchanged source rows. Source assets are referenced, not copied: retain the absolute source release folders listed in the manifest. No annotations were rerun, no latest pointer was changed, and no training was launched.\n'
