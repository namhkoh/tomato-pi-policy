...
TELEMETRY=Path('D:/isaac-sim-6.0.1/extscache/omni.kit.telemetry-0.5.2+f9bf0dda.wx64.r.cp312/omni.telemetry.transmitter/omni.telemetry.transmitter.exe')
AUX_BINDINGS={str(TELEMETRY):'43575e174a8c5af632a82cb505b0e61e5981a7911820e96fc30a562a9a4ef326'}


def checked_telemetry(row,native):
    import re
    value=identity(row)
    verify_bindings(AUX_BINDINGS)
    require(value['Name'].lower()=='omni.telemetry.transmitter.exe'
        and Path(value['ExecutablePath']).resolve()==TELEMETRY.resolve()
        and value['ParentProcessId']==native['ProcessId'],
        'Only pinned telemetry directly parented by exact owned Kit can register')
    argv=command_argv(value['CommandLine'])
    require(argv and Path(argv[0]).resolve()==TELEMETRY.resolve(),'Telemetry actual command executable differs')
    birth=re.fullmatch(r'/Date\((\d+)\)/',value['CreationDate'])
    parent_birth=re.fullmatch(r'/Date\((\d+)\)/',native['CreationDate'])
    require(birth and parent_birth and int(birth[1])>=int(parent_birth[1]),
        'Telemetry must be created after the exact owned native parent')
    return value


def register_telemetry(rows,coordinator,slots):
    import copy
    result=copy.deepcopy(slots)
    require(same_identity(one(rows,coordinator['ProcessId']),coordinator),'Coordinator identity changed')
    for slot in result:
        command,native=slot['command_identity'],slot['native_identity']
        require(same_identity(one(rows,command['ProcessId']),command)
            and same_identity(one(rows,native['ProcessId']),native),'Cannot register auxiliary of exited/reused parent')
        candidates=[r for r in rows if r['ParentProcessId']==native['ProcessId']
            and r['Name'].lower()=='omni.telemetry.transmitter.exe']
        require(len(candidates)<=1,'Unexpected multiple owned telemetry transmitters')
        slot['auxiliary_identities']=[checked_telemetry(r,native) for r in candidates]
    return result


def owned_allowlist(coordinator, slots):
    """Slots are immutable command+native receipts from no more than four launches."""
    require(len(slots)<=4 and len({s['slot'] for s in slots})==len(slots)
        and all(s['slot'] in (0,1,2,3) for s in slots),'Exactly bounded unique slots required')
    ...
    for slot in slots:
        ...
        require(isinstance(auxiliaries,list) and len(auxiliaries)<=1,'Bounded exact telemetry auxiliaries required')
        result.extend(checked_telemetry(row,native) for row in auxiliaries)
...
def classify_owned(rows, allowed, base_report, *, require_all=True):
    ...
    optional_auxiliary_pids=set()
    for pid,value in expected.items():
        if value['Name'].lower()=='omni.telemetry.transmitter.exe':
            parent=expected.get(value['ParentProcessId'])
            require(parent is not None and parent['Name'].lower()=='kit.exe'
                and Path(parent['ExecutablePath']).resolve()==KIT.resolve(),
                'Optional auxiliary must still name an exact registered owned Kit parent')
            checked_telemetry(value,parent)
            optional_auxiliary_pids.add(pid)
    for pid,value in expected.items():
        require(pid in by or not require_all or pid in optional_auxiliary_pids,
            'Owned command/native/coordinator process disappeared before admission')
