...
SCHEMA = 'greenhouse.native848_direct_camera_batch_plan.v1'
SAMPLE_SCHEMA = 'greenhouse.native848_direct_camera_sample.v1'
RESULT_STATE = 'native848_direct_batch_captured_pending_independent_audit'
PROFILE = 'native848_direct_cached_poses_first56_then8.v1'
...
def implementation_bindings():
    ...
    pending = [root / name for name in (
        'native848_direct_plan_v1.py', 'native848_direct_worker_v1.py',
        'native848_direct_audit_v1.py', 'native848_direct_admission_v1.py')]
    found = {}
    while pending:
...
def prepare(cache_path):
    ...
    plan = dict(schema=SCHEMA, cache_path=str(path), cache_sha256=sha256(path),
        ...
        long_reference_probe_ids=[selected[i] for i in (1, 13, 25)],
        instance_backend='fast', max_frames=30, full_greenhouse_stage_count=1,
...
def check(plan, *, full=False):
    ...
    require(plan['schema'] == SCHEMA and plan['resolution'] == [848, 408]
        ...
        and plan['max_frames'] == 30 and plan['full_greenhouse_stage_count'] == 1
    ...
    require(len(ids) == len(set(ids)) == 30 and set(ids) == set(records), 'Incomplete bounded camera batch')
    require(plan['long_reference_probe_ids'] == [ids[i] for i in (1, 13, 25)], 'Changed render probes')
    plans = {}
    generated = None
