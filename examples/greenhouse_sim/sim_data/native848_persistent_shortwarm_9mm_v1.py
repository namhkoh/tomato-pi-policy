...
EVALUATION_SCHEMA = "greenhouse.native848_persistent_shortwarm_evaluation.v1"
...
def evaluate_segment(capturepath,output):
    ...
    from . import native848_persistent_shortwarm_capture_v1 as producer
    ...
    require(set(r['observation_id'] for r in result['frames'])<={authority['scheduled']['selected_sample_id']},'Wrong diagnostic camera')
    ...
    try:
        ...
        value=dict(schema=EVALUATION_SCHEMA,source_capture=str(capture),
            ...
            frames_evaluated=len(summaries),diagnostic_only=True,production_profile_qualification=False,
...
def validate_registry(request_pin, identity_pin, batch, scheduled, registered, values, contexts, observations, sources, profiles, producer):
    ...
    require(batch['schema']==producer.RESULT_SCHEMA and batch['request']==request_pin
        ...
        and batch['diagnostic_only'] is True,
...
def authenticate_batch(capture,read,local,producer):
    ...
    require(batch['diagnostic_profile']==request['diagnostic_profile'] and complete['diagnostic_only'] is True,
    ...
    for i,(spec,entry,(source,candidates)) in enumerate(zip(request['segments'],batch['segments'],sources)):
        ...
        require(context['profile_role']=='source_optical_settings_and_baseline_warmup_reference_only'
            ...
            and value['selected_diagnostic_sample_id']==context['selected_diagnostic_sample_id']==spec['selected_sample_id']
            and len(obslist)<=1 and all(o['sample_id']==spec['selected_sample_id'] for o in obslist),
            'Diagnostic actual profile or selected camera differs')
    ...
    return dict(complete=complete,segment_result=value,source_request=source,candidates=candidates,source_bindings=bindings,scheduled=request['segments'][i],
        public=dict(batch_owner_complete=local(trial/'owner_complete.json'),batch_result=complete['result'],batch_request=complete['request'],
            ...
            all_batch_segments_and_global_clocks_authenticated=True,diagnostic_only=True,
            actual_diagnostic_profile=contexts[i]['actual_diagnostic_profile'],diagnostic_profile=request['diagnostic_profile']))
