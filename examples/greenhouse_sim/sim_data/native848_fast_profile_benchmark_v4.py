"""Opt-in native848 DLSS8 and compact lossless advancing-clock benchmark; no TRAIN admission.
...
SCHEMA = 'greenhouse.native848_fast_profile_benchmark_plan.v4'
SAMPLE = 'greenhouse.native848_fast_profile_benchmark_sample.v4'
...
PROFILE = 'rt2_dlss_scheduler_1over60_compact_lossless_reset8.v1'
...
def prepare():
    plan = dict(schema=SCHEMA, resolution=[848, 408], budgets=BUDGETS,
        ...
        full_scene_observations=4, control_observations=5,
...
def check(plan, full=False):
    ...
    require(plan['schema']==SCHEMA and plan['resolution']==[848,408] and plan['budgets']==BUDGETS
        ...
        and plan['full_scene_observations']==4 and plan['control_observations']==5
...
def recording_writer(rep,unrequested_folder):
    ...
    class DiagnosticWriter(rep.Writer):
        def __init__(self):
            self.version='native848-fast-profile-benchmark-v4'
...
def full_scene(app, output, plan, scene_name, loaded):
    ...
    try:
        for item in plan['schedules'][scene_name]:
            ...
            print('FAST848_PROFILE_V4',scene_name,item['name'],receipt.get('capture_valid',False),receipt['passed_actual_automated_criteria'],flush=True)
    finally:
...
def capture(app,output,plan):
    ...
    return dict(state=STATE,scenes=scenes,controls=control,budget_status=status,
        ...
        elapsed_seconds=time.perf_counter()-started,full_scene_observations=4,
...
def audit(output,plan):
    ...
    require(request['plan_sha256']==result['plan_sha256']==sha256(request['plan_path'])
        ...
        and result['equality_used_for_acceptance'] is False and result['full_scene_observations']==4,'Changed execution scope')
    ...
    actual_files={str(p.resolve()) for p in output.rglob('*') if p.is_file() and p.name not in ('result.json','evidence_bindings.json','audit_fast_profile_v4.json')}
    ...
    try:
        ...
        require([r['name'] for r in controls_rows]==[f'b{n}_{phase}' for n in BUDGETS for phase in plan['control_order']], 'Changed five-control sequence')
        ...
        for scene_name,rows in result['scenes'].items():
            require([{k:r[k] for k in ('name','budget','sample_id','role')} for r in rows]==plan['schedules'][scene_name], 'Changed four-frame schedule')
...
def cpu_tests():
    ...
    require(len(schedule('transition'))==3 and len(schedule('negative'))==1,'Wrong transition budget');checked+=1
    import tempfile
    from .native_instances import normalize_payload
    with tempfile.TemporaryDirectory(prefix='fast848_profile_v4_cpu_',dir=D) as temporary:
...
def main(argv=None):
    ...
    if args.action=='audit':
        ...
        require(not (output/'audit_fast_profile_v4.json').exists(),'Create-only audit required')
        write_json(output/'audit_fast_profile_v4.json',report);return
