...
def check_parallel(value,*,rows=None):
    ...
    if result['schema']=='greenhouse.native848_multianchor_parallel.v1':
...
def check_serial(value,*,rows=None):
    ...
    bindings={}
    def pinned(item):
        p=Path(item['path']).resolve();require(p.is_relative_to(ROOT) and sha256(p)==item['sha256'],'Changed serial predecessor evidence')
    ...
    require(trial.is_relative_to(D) and paths['owned_exit']==trial/'owned_exit.json'
