"""Fully occupied original greenhouse slots, complete same-split source plants.
...
SCHEMA='greenhouse.native848_fully_labeled_scene_policy.v1'
CENSUS_SCHEMA='greenhouse.native848_fully_labeled_census.v1'
...
def policy(split):
    ...
    return dict(schema=SCHEMA,dataset_split=split,total_plant_slots=144,
        ...
        native_instanceable_plants=False,source_assets_modified=False)
...
def populate_labeled(stage,package,anchor,source_plan,reports,expected_policy):
    ...
    for index,slot in enumerate(slots):
        ...
        census.append(dict(plant_root=path,variant_id=variant['variant_id'],source_family=family,source_plant_id=family,
            source_split=split,authenticated_component_plant=True,component_count=len(record['component_paths']),
    ...
    require(len(active)==144 and all(p.IsActive() and not p.IsInstanceable() for p in active),'Full ordinary plant population required')
