...
Frozen v1 CPU decisions are complete only after their newline-terminated log
...
SCHEMA='greenhouse.original848_bulk_live_prefix_closure.v2'
...
def check_observation(obs,row,native,item,ctx,profile,pins):
    ...
    key=geometry_pose_key(ctx['scene_revision'],pose,fingerprint(ctx['source_bindings']))
    require(obs['geometry_proof']['pose_key']==proof['pose_key']==key
        and proof['scene_revision']==ctx['scene_revision']
        and proof['source_bindings_sha256']==fingerprint(ctx['source_bindings'])
...
def geometry_hold_proof(capture,ctx,cached,pins):
    ...
    for path in Path(ctx['geometry_proofs_directory']).glob('*.json'):
        ...
        if (value['joint_degrees']==cached['joint_degrees'] and
            ...
            require(value['pose_key']==geometry_pose_key(ctx['scene_revision'],pose,fingerprint(ctx['source_bindings']))
                and value['scene_revision']==ctx['scene_revision']
                and value['source_bindings_sha256']==fingerprint(ctx['source_bindings'])
...
def prepare_review_prefix(context_path,context_sha256,admissions,prior,prior_sha256,output,
    ...
    if '--ticket' in command:
        require(intent['native_workers']==2 and intent['automatic_expansion'] is False
            and intent['planned_frames']>=len(plan['schedule']),'Unexpected parallel coordinator scope')
        ...
        require(ticket['schema']=='greenhouse.original848_two_worker_ticket.v1' and ticket['slot'] in (0,1)
            and ticket['plan']['sha256']==ctx['plan_sha256']
    ...
    if capture_start:
        ...
        require(previous_receipt['schema']==SCHEMA and previous_receipt['context_sha256']==context_sha256
    ...
    result=dict(schema='greenhouse.original848_bulk_live_prefix_review.v2',
