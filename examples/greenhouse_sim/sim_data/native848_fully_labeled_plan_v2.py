...
SCHEMA='greenhouse.native848_fully_labeled_plan.v2'
...
def _source(spec):
    ...
    for pin,(cache,part) in zip(value['caches'],batches):
        ...
        require(anchor['expected_robot_snapshot']['camera_to_head_column_vectors']==reference['expected_robot_snapshot']['camera_to_head_column_vectors'],'Multi-anchor calibrated head mount differs')
...
def validate_prior_schedule(spec):
    ...
    for pin in value['prior_plans']:
        ...
        plan=read_json(pp);require(plan['schema'] in ('greenhouse.native848_fully_labeled_plan.v1',SCHEMA),'Prior schedule from another population epoch')
