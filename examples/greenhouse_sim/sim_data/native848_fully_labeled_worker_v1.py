...
import traceback
import numpy as np
from . import native848_fully_labeled_plan_v1 as api
...
def capture(app, output, plan_path, plan, checked):
    ...
    try:
        ...
        context = dict(**population_metadata(checked['capture_split'],native=True), accepted_training_increment=0, schema=api.CONTEXT_SCHEMA, plan_path=str(plan_path.resolve()), plan_sha256=sha256(plan_path),
            ...
            process_startup_timing=checked['process_startup_timing'],
...
def native_command(args):
    return ['D:/isaac-sim-6.0.1/python.bat','-B','-u','-m','sim_data.native848_fully_labeled_worker_v1',
...
def record_native_identity(args):
    ...
    return dict(schema='greenhouse.native848_fully_labeled_worker_identity.v1',owner_identity=owner,
...
def validate_native_identity(value,command,*,owner_identity=None):
    require(value['schema']=='greenhouse.native848_fully_labeled_worker_identity.v1'
        and value['same_snapshot_identity_checks'] is True and value['command']==command
        and command[1:5]==['-B','-u','-m','sim_data.native848_fully_labeled_worker_v1'],
...
def runtime_evidence(phase,config,native_pin):
    ...
    return dict(schema='greenhouse.native848_fully_labeled_runtime_evidence.v1',phase=phase,
...
def validate_optout_capture(capture,value,context):
    ...
    for phase in ('start','end'):
        ...
        require(evidence['schema']=='greenhouse.native848_fully_labeled_runtime_evidence.v1'
