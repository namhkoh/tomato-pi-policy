"""Diagnostic A7/B7/A1/B1 in one persistent native process; four replay frames, zero accepted increments.
...
from . import native848_persistent_shortwarm_plan_v1 as diagnostic
MODULE='sim_data.native848_persistent_shortwarm_capture_v1'
REQUEST_SCHEMA='greenhouse.native848_persistent_shortwarm_capture_request.v1'
RESULT_SCHEMA='greenhouse.native848_persistent_shortwarm_batch_capture.v1'
SEGMENT_SCHEMA='greenhouse.native848_persistent_shortwarm_segment_capture.v1'
OBSERVATION_SCHEMA='greenhouse.native848_persistent_shortwarm_native_observation.v1'
FLAGS=dict(baseline.FLAGS,diagnostic_only=True)
pin=baseline.pin
...
def implementation_bindings():
    names=('native848_persistent_foreground_v1.py','native848_controlled_capture_v2.py','native848_persistent_capture_v1.py','native848_persistent_shortwarm_plan_v1.py')
...
def check_request(request):
    ...
    require(len(request['segments'])==4,'Exactly A7/B7/A1/B1 required')
    cache={};segments=[]
...
def make_request(source_a,source_b,sample_a=None,sample_b=None):
    sources=[bound(x) for x in (source_a,source_b)]
    candidates=[baseline.check_request(x) for x in sources]
    samples=[sample_a or candidates[0]['records'][0]['sample_id'],sample_b or candidates[1]['records'][0]['sample_id']]
    specs=[dict(segment_id=f's{i:03d}',source_request=(source_a,source_b)[i%2],
        selected_sample_id=samples[i%2],warmup_request_count=diagnostic.COUNTS[i]) for i in range(4)]
    value=dict(schema=REQUEST_SCHEMA,segments=specs,max_frames=4,
        diagnostic_profile=diagnostic.make_profile(candidates[0]['profile']),
...
def validate_batch_registry(result, request, capture_root, *, request_pin, native_identity):
    ...
    require(result['schema']==RESULT_SCHEMA and result['request']==request_pin
        ...
        and result['diagnostic_only'] is True and result['accepted_training_increment']==0
        and result['diagnostic_profile']==request['diagnostic_profile'],
    ...
    for scheduled,registered in zip(request['segments'],result['segments']):
        ...
        for frame in value['frames']:
            ...
            require(name==scheduled['selected_sample_id'],'Wrong diagnostic camera')
            ...
            require(context['profile_role']=='source_optical_settings_and_baseline_warmup_reference_only'
                and context['actual_diagnostic_profile']==value['actual_diagnostic_profile']
                and len(value['actual_diagnostic_profile']['warmup_steps'])==scheduled['warmup_request_count'],
...
def capture_segment(app, request, candidates, output, *, scheduled, source_request_pin, batch_request_pin, segment_id, session, writer, batch_native_identity, previous, anchor_cache):
    ...
    records=[r for r in candidates['records'] if r['sample_id']==scheduled['selected_sample_id']]
    require(len(records)==1,'Exactly selected diagnostic camera required')
    ...
    context=dict(schema='greenhouse.native848_generated_population_context.v1',dataset_split='train',
        ...
        actual_diagnostic_profile=actual,selected_diagnostic_sample_id=scheduled['selected_sample_id'],
    ...
    with ExitStack() as cleanup:
        ...
        for record in records:
            if len(committed)>=1:break
            tick=time.perf_counter()
            pose,cal=apply_cached_pose(stage,robot,record)
            timings['pose_application_seconds']=time.perf_counter()-tick
            tick=time.perf_counter()
            screen=geometry()
            timings['collision_screen_seconds']=time.perf_counter()-tick
            ...
            timings['current_9mm_geometry_seconds']=time.perf_counter()-tick
            tick=time.perf_counter()
            ...
            timings['workspace_check_seconds']=time.perf_counter()-tick
            ...
            timings['production_static_begin_reset_seconds']=time.perf_counter()-tick
            tick=time.perf_counter()
            payload,e=primitives.request_once(rep,writer,profile['request_subframes'],profile['delta_time_seconds'])
            timings['production_request_seconds']=time.perf_counter()-tick
            tick=time.perf_counter()
            after=monitor.token()
            ...
            timings['production_validation_serialization_seconds']=time.perf_counter()-tick
            print('CONTROLLED_FRAME_SAVED',sample_id,flush=True)
        tick=time.perf_counter()
        receipts=sink.close()
        timings['sink_drain_seconds']=time.perf_counter()-tick
        tick=time.perf_counter()
        require(len(receipts)==len(committed)<=1 and writer.capture_error is None, 'Capture callback/write mismatch')
        if not warmed:
            frozen_sources=source_hashes(stage)
            for path,h in frozen_sources.items():
        ...
        save_json(output/'result.json',dict(schema=SEGMENT_SCHEMA,state='captured_pending_complete_annotation_and_visual_review',
            ...
            morphology_id=morphology,actual_diagnostic_profile=actual,selected_diagnostic_sample_id=scheduled['selected_sample_id'],original_143_backgrounds_preserved=True,**FLAGS))
...
def capture(app,request,segments,output,*,request_pin):
    ...
    with ExitStack() as cleanup:
        ...
        result=dict(schema=RESULT_SCHEMA,state='captured_pending_complete_annotation_and_visual_review',
            ...
            total_capture_seconds=time.perf_counter()-started,background_closure=background_closure,diagnostic_profile=request['diagnostic_profile'],**FLAGS)
