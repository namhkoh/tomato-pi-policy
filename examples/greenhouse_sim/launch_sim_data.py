...
def main():
    ...
    if args.output is None:
        name = ("phase1_review_" + datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ_") + uuid.uuid4().hex[:6]
                if args.review or args.review_plant else "preview_20260905")
    ...
    robot = None if args.no_robot else add_robot_preview(stage, gutter_x=cx)
    ...
    with controls.frame:
        with ui.VStack(spacing=6):
            ...
            ui.Label("v1.2 robot + D405 RGB preview (848x408). Wrist adapters are simulator designs. Cutting, grasping and dynamics are not enabled in this package preview.",
                     word_wrap=True)
