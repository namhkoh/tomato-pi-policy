...
class NativeStartupClearance:
    def __init__(self,query_box,query_sphere,scene_bounds,robot_paths,*,guard,
                 floor_root=None,max_queries=20000,wall_limit_s=60.):
    ...
    def _box(self,centre,axes,half):
        ...
        return self._query(self.box_query,centre,axes,half+reserve)
        ...
        for path,body,link,kind,shape in shapes:
            ...
            if kind=='capsule':
                ...
                for centre in centres:
                    hit=self._foreign(self._query(self.sphere_query,centre,expanded),link)
                    if hit:break
            elif kind=='box':
                centre,axes,half=shape
                hit=self._foreign(self._box(m[:3,:3]@centre+m[:3,3],m[:3,:3]@axes,np.asarray(half)+margin),link)
