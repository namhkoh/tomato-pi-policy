...
"""Gemma adaptation for Pi05 subtask generation, taken from big_vision.

This module differs from gemma.py in its KV cache management:
- Uses (idx, k_cache, v_cache) format for autoregressive token generation
- Supports _init_cache / _update_cache for step-by-step decoding
- Adds deembed() method for token prediction logits
...
class Attention(nn.Module):
    """Attention module with autoregressive KV cache support."""

    configs: Sequence[Config]

    def _init_cache(self, k, v, cache_size):
        """Initialize KV cache with fixed size for autoregressive decoding."""
    ...
    def _update_cache(self, k, v, idx, k_cache, v_cache):
        """Update KV cache with a single new token via dynamic_update_slice."""
    ...
    def __call__(self, xs, positions, attn_mask, kv_cache):
        ...
        if kv_cache is None:
            ...
            if k.shape[1] == 1:
                # Next token prediction: update cache with single token
                idx, k_cache, v_cache = self._update_cache(k, v, idx, k_cache, v_cache)
                k, v = k_cache, v_cache
            else:
                # Action sampling: concat with cache but don't update it
...
class Module(nn.Module):
    """Transformer model with autoregressive KV cache for subtask generation."""
